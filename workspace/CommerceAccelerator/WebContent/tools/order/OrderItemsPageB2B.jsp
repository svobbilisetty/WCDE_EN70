	...
	selOrgId = orderBean.getOrganizationId();
       ...
       if (itemList != null) {
              for (int i=0; i<itemList.getResultSetSize(); i++) {
                     runningTotal = runningTotal.add(itemList.getOrderItemListData(i).getTotalProductInEJBType());
...
aStrBuffer.append("'/>");
			if(isEditable && !isFree){	
                            ...
                            aStrBuffer.append(getFormattedQuantity(itemList.getOrderItemListData(i).getQuantityInEJBType().doubleValue(), jLocale));
                       }else{
...
<%=comm.addDlistColumn(	getFormattedAmount(itemList.getOrderItemListData(i).getPriceInEJBType(),orderCurrency,	new Integer(langId),storeId.toString()),"none")%> 
