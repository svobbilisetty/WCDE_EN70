...
function shipIn1stOrder() {
	...
	if (checkedItems.length > 0) {
		...
		if (get2ndOrderId() == "")
			document.splitActionForm.URL.value="OrderItemsAllocationPageB2C?ActionXMLFile=order.orderItemsAllocationPage&listsize=15&startindex=0&firstOrderId="+get1stOrderId();
		else
			document.splitActionForm.URL.value="OrderItemsAllocationPageB2C?ActionXMLFile=order.orderItemsAllocationPage&listsize=15&startindex=0&firstOrderId="+get1stOrderId()+"&secondOrderId="+get2ndOrderId();
	...
	if (confirmDialog("<%= UIUtil.toJavaScript( (String)orderMgmtNLS.get("addProductDialogRemoveConfirm")) %>")) {
		...
		if (checkedItems.length > 0) {				
			...
			if (validatePanelData()) {
				...
				document.callActionForm.URL.value="OrderItemsAllocationPageB2C?ActionXMLFile=order.orderItemsAllocationPage&listsize=15&startindex=0&firstOrderId="+get1stOrderId()+"&secondOrderId="+get2ndOrderId();	
...
<form name="itemListForm">
	...
	%>
		...
		<%                  
			...
			<%= comm.addDlistColumn(getFormattedQuantity(itemList.getOrderItemListData(i).getQuantityInEJBType().doubleValue(), jLocale), "none") %>
			<% if (itemList.getOrderItemListData(i).getEstimatedAvailableTimeInEJBType() != null) { 
			 	String estimatedShipDate = TimestampHelper.getDateFromTimestamp(itemList.getOrderItemListData(i).getEstimatedAvailableTimeInEJBType(), jLocale);                                
			%>		           
