...
<%! 
	...
	public double getOrderSubTotal(OrderDataBean tmpOrder) {
		...
		try {
			...
			total = tmpOrder.getTotalProductPriceInEJBType().doubleValue();
			total = total + discount;
...
<html>
  <head>  
	...
	if (defined(preCommand) && preCommand != "") {
     ...
     // for first order
     // JY: do not need to create the first order
...
//alert("1" + parent.parent.modelToXML("XML"));
	...
	<% 
      ...
      function recalculate()
      {
		...
		if (("<%=firstOrderExist%>" == "true") && ("<%=secondOrderExist%>" == "true")) {
			document.callActionForm.URL.value="OrderAdjustmentContentB2C?<%= ECOptoolsConstants.EC_OPTOOL_FIRSTORDER_ID %>=<%= firstOrderId %>&<%= ECOptoolsConstants.EC_OPTOOL_SECONDORDER_ID %>=<%= secondOrderId %>";
		} else if (("<%=firstOrderExist%>" == "true")) {
		    	document.callActionForm.URL.value="OrderAdjustmentContentB2C?<%= ECOptoolsConstants.EC_OPTOOL_FIRSTORDER_ID %>=<%= firstOrderId %>";
		} else {
		    	document.callActionForm.URL.value="OrderAdjustmentContentB2C?<%= ECOptoolsConstants.EC_OPTOOL_SECONDORDER_ID %>=<%= secondOrderId %>";
		}
	...
	%>
      ...
      function displayOrderAdjustments(value)
      {
      	...
      	var url = "/webapp/wcs/tools/servlet/OrderAdjustmentContentB2C?"+
	...
	%>
		...
		%>
			<tr class="<%= UIUtil.toHTML(classId) %>">
				...
				<td class="list_info1" align="left">
					<%= UIUtil.toHTML(getFormattedQuantity(afirstOrderItems[i].getQuantityInEJBType().doubleValue(), jLocale)) %>
				</td>
 	...
 	<% if (secondOrderExist && 
		...
		%>
			<tr class="<%=classId%>">
				...
				<td class="list_info1" align="left">
					<%= UIUtil.toHTML(getFormattedQuantity(asecondOrderItems[i].getQuantityInEJBType().doubleValue(), jLocale)) %>
				</td>
				<td class="list_info1" align="right">
