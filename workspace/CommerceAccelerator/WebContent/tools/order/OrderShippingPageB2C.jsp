...
<% 
	...
	* ---- */
	...
	// check shipping/billing address used for place guest order
	
	boolean allItemHasAddress = true;
	
	CSROrderItemDataBean[] csrOrderItemDBs = itemList.getOrderItemList();
	CSROrderItemDataBean csrOrderItem = null;
	if(csrOrderItemDBs !=null){
		for(int i=0; i< csrOrderItemDBs.length; i++){
	     	 csrOrderItem = csrOrderItemDBs[i];
	     	 String addressId = csrOrderItem.getAddressId();
	      	if(addressId == null || addressId.trim().length()<=0){
	     		 allItemHasAddress = false;
	     	 	break;
	     	 }
	 	 }	
	}
	// end check shipping/billing address
...
function validatePanelData() { 
	...
	if( "<%=allItemHasAddress%>" =="false"){
		   alertDialog("<%= UIUtil.toJavaScript( (String)orderMgmtNLS.get("shipAddressMustBeAddedMsg")) %>");
		   return false;
	}
