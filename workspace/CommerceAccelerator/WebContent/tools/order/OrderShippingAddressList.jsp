...
<form name="ShippingAddressListFORM" method="post">
	...
	%>
		<%= comm.startDlistRow(rowselect) %>
			...
			<%= comm.addDlistColumn(nickName.elementAt(i).toString(), "none") %>
			<%= comm.addDlistColumn(lastName.elementAt(i).toString(), "none") %>
			<%= comm.addDlistColumn(firstName.elementAt(i).toString(), "none") %>
			<%
			...
			%>			
			<%= comm.addDlistColumn(formatAddress, "none") %>		
		<%= comm.endDlistRow() %>
