...
<% 
	...
	try{
		abStoreEntityCalUsage.refreshCopyHelper();
		strDefaultSalesTaxCode = abStoreEntityCalUsage.getCalculationCodeId();
		...
		strDefaultSalesTaxCode = "";
	}
	
	if(strDefaultSalesTaxCode == null) {
		strDefaultSalesTaxCode = "";
	} 
%>
  <%--
    ...
    var defcalcodeid = null; //default position in the option list
   	var	remCalCode1 = parent.get("remCalCode1");  
	if(remCalCode1 == null){
		remCalCode1 = new Vector();
	} 
...
//    }
//    if (calcodes.size()==0) {
//    	alertDialog("<%=taxNLS.getJSProperty("noSalestaxCode")%>");
//    }	
//    else 
//    numOfTaxCodes = calcodes.size();
...
//         alertDialog("<%=taxNLS.getJSProperty("AddDefaultSalestaxCode")%>");
//        orgtext = document.f1.definedCodes.options[0].text
//         document.f1.definedCodes.options[0].text = addDefaultText(orgtext) 
//         catencalcd.elementAt(0).calculationCodeId = document.f1.definedCodes.options[0].value; 
//    }
//    dumpGlobalVarialbes();
...
//  }
    ...
    for (var i=0; i < jurst.container.length; i++) {
       ...
       for (var k=0; k < taxcgry.container.length; k++){
		...
		var tempcalrlookup = getCalrlookup(thisjurst.jurisdictionId, thistaxcgry.categoryId);             
	    	//alert("tempcalrlookup="+tempcalrlookup.value);
  ...
  <%--
    ...
    --%>
  function setDefaultSalestaxCode()
  {
...
//		alertDialog("defaultindex found ="+i);
    ...
    for ( var i=0; i<=document.f1.definedCodes.options.length-1; i++ ) {
      if ( document.f1.definedCodes[i].selected == true ) {
        ...
        //	temp.catalogEntryCalculationCodeId = "-3";
    ...
    parent.put("defaultSalesTaxCode", defcalcodeid);

    <%-- if nothing was selected, prompt the user to select something. --%>
    if(!selectionMade) {
      alertDialog ("<%=taxNLS.getJSProperty("SelectSalesTaxCodeToSetDefault")%>");
      return;
    }
  }  
...
//    }        
    ...
    for(var i = document.f1.definedCodes.length-1; i >= 0; i--) {
      if(document.f1.definedCodes[i].selected == true) {
        ...
        if (selectedindex == defaultindex){
        	alertDialog("<%=taxNLS.getJSProperty("Can'tRemoveDefaultSalestaxCode")%>");
        	return;
        }
...
//	}
        removeElementAt(calcdindex,calcodes);
          ...
          if (calcode_id.substring(0,1) != "@"){
             //remCalCode1.addElement(calcode_id);
             addElement(calcode_id,remCalCode1);
             
        }
          
        //if the removed calcode is set as default, clear it.  
        if(calcode_id == parent.get("defaultSalesTaxCode")) {
        	parent.put("defaultSalesTaxCode", "");
        }
...
//         }
    ...
    parent.put("remCalCode1",remCalCode1);
    <%-- If nothing was selected, prompt the user to select something. --%>
    if (!selectionMade) {
...
<FORM NAME="f1" onsubmit='return false;'>
  <H1><%=taxNLS.getProperty("taxSalesTaxCodesTitle")%></H1>
  <%=taxNLS.getProperty("SalesCodeMsg")%>
  <BR><BR>
  	<%=taxNLS.getProperty("NewSalesTaxCode")%>
			  <TABLE BORDER=0>
			    <TR>
				   <TD VALIGN=TOP>
		  ...
		  <BR><BR>
        <%=taxNLS.getProperty("DefinedSalesTaxCodes")%>
		  <TABLE>
		   <TR>
			  <TD rowspan="2">
 			  ...
 			  <TD valign="TOP" height="21">
 			  	<INPUT type="button" name="defaultButton" id="nbp" class="button" value='<%=taxNLS.getProperty("SalesTaxCodesDefaultButton")%> ' onclick="setDefaultSalestaxCode()">
 			  </TD>
 		   </TR>
 		   <TR> 
