...
function languageListSelect(selSelectedObject){
	...
	var urlList = "/webapp/wcs/tools/servlet/GuidedSellAnswerListView?ActionXMLFile=guidedSell.GuidedSellAnswerList";
		urlList =urlList+"&<%=GuidedSellUIConstants.GS_CONCEPT_ID%>="+<%=conceptId%>;
		urlList =urlList+"&<%=GuidedSellUIConstants.GS_LANGUAGE_ID%>="+selValue;
		urlList =urlList+"&<%=GuidedSellUIConstants.GS_CATEGORY_ID%>="+<%=categoryId%>;
		urlList =urlList+"&<%=GuidedSellUIConstants.GS_METAPHOR_ID%>="+<%=metaphorId%>;
		urlList =urlList+"&<%=GuidedSellUIConstants.GS_TREE_ID%>="+<%=treeId%>;
	    gsframe.basefrm.location.replace(urlList);
	}
}

function loadPanelData(){
	gsModifyBCT(<%=gsLanguageId%>);
...
<FRAMESET FRAMESPACING="0" BORDER="0" FRAMEBORDER="0" ROWS="20%,80%" ONLOAD="loadPanelData();">
	<FRAME SRC="<%=urlLanguage%>" name="language" scrolling="no" title="<%=guidedSellRB.get(GuidedSellUIConstants.GS_LANGUAGE_FRAME)%>"/>
	<FRAME SRC="<%=urlList%>" name="gsframe" title="<%=guidedSellRB.get(GuidedSellUIConstants.GS_ANSWER_LIST_FRAME)%>"/>
</FRAMESET>
</HTML>