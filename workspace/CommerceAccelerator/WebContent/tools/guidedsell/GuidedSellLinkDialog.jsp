...
<%
	...
	String fromPage = request.getParameter(GuidedSellUIConstants.GSMLL_FROM_PAGE);
	String forChange = request.getParameter(GuidedSellUIConstants.GSMLL_FOR_CHANGE);
...
function savePanelData(){
	...
	var xml = '<LINKDATA>\n';
	xml = xml + getDefaultXMLData('<%=categoryId%>','<%=metaphorId%>','<%=gsLanguageId%>','<%=treeId%>','<%=conceptId%>','<%=fromPage%>','<%=forChange%>');
...
<SCRIPT LANGUAGE="JavaScript">
	...
	if(changedObject != null){
		...
		var linkURL = "<%=urlSelectPrefix%>&<%=GuidedSellUIConstants.GSMLL_METAPHOR_LINK_NAME%>="+linkName;
		var selectURL = "<%=urlListPrefix%>&<%=GuidedSellUIConstants.GSMLL_METAPHOR_LINK_NAME%>="+linkName;
