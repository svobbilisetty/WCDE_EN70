...
DTreeAbstractNode.prototype.loadChildNodes = function () {
	...
	dframe.location.replace(webappPath + 'DynamicTreeFetchDataView?' + this.childrenUrlParam + '<%= NVPs.toString() %>');
...
function gotoDataFrame(path, searchBy)  {
	if (searchBy == "name") {
		DTreeHandler.isLoading = true;
		dframe.location.replace(webappPath + 'DynamicTreeFetchDataView?<%= NVPs.toString() %>&gotoNodeByName=' + path);
	}
	else if (searchBy == "value") {
		DTreeHandler.isLoading = true;
		dframe.location.replace(webappPath + 'DynamicTreeFetchDataView?<%= NVPs.toString() %>&gotoNodeByValue=' + path);
	}
}
...
<%}%>
	...
	var dataFrame = '<iframe name="dframe" id="dataframe" width="0" height="0" src="' + webappPath + 'DynamicTreeFetchDataView?<%= tree.getInitDataURLParam() %><%= NVPs.toString() %><%if (request.getParameter("gotoNode")!=null) { %>&gotoNode=<%=(String)request.getParameter("gotoNode")%><%}%>"></iframe>';
