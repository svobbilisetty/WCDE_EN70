...
<frameset framespacing="0" border="0" frameborder="0" rows="20%, 80%" onload="init()">
   <frame name="titleFrame" id="titleFrame" src="/webapp/wcs/tools/servlet/ContractShippingChargeAdjustmentPanelTitleView?contractId=<%=tradingId%>&base_contract_id=<%=base_contract_id%>&accountEdit=<%=editingAccount%>&accountId=<%=accountId%>&delegationGrid=<%=delegationGrid%>" title="<%=UIUtil.toJavaScript((String)contractsRB.get("catalogTreeTitlePanel"))%>">
