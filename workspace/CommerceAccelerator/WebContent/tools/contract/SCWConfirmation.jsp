...
function generateInstructions(){
	var htmlText = null;	
        ...
        htmlText += "</TD>";
        htmlText += "<TD>";        
        htmlText += changeSpecialText("<%=UIUtil.toHTML((String)resourceBundle.get("confirmationInstruction1"))%>", '<A href="javascript:bookmark();">', '</A>' );
        htmlText += "</TD>";
        htmlText += "</TR>";        
        ...
        if(<%=isFromAccelerator%>){    
        	htmlText += "<%=UIUtil.toHTML((String)resourceBundle.get("confirmationInstruction2"))%>" + '&nbsp;' + store_url + '';
        } else {
        	htmlText += "<%=UIUtil.toHTML((String)resourceBundle.get("confirmationInstruction2"))%>" + '&nbsp;<A href="javascript:launchStore();">' + store_url + '</A>';
        }
        htmlText += "</TD>";
        htmlText += "</TR>";        
...
<table id="SCWConfirmation_Table_2">
	<tr>
		<td id="SCWConfirmation_TableCell_2">
			<button type="button" value="launchBookmark" onclick="bookmark()" class="general"> <%=UIUtil.toHTML((String)resourceBundle.get("launchBookmark"))%></button>
		</td>
		<%
		if(isSeparateWindowLaunched){
		%>
