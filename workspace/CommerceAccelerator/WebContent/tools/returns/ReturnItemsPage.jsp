...
<%
	...
	rmaItemLists = rmaItemListDB.getRmaItemDataBeans();
	    ...
	    for (int i=0; i<rmaItemLists.length; i++) {
	        runningTotal = runningTotal.add(rmaItemLists[i].getCreditAmountInEJBType());
	        runningTotal = runningTotal.add(rmaItemLists[i].getAdjustmentCreditInEJBType());
	        runningTotal = runningTotal.add(rmaItemLists[i].getAdjustmentInEJBType());
...
<FORM NAME="ReturnItemsListForm">
    ...
    <%= comm.startDlistTable(UIUtil.toJavaScript( (String)returnsNLS.get("orderItemListTableSummary"))) %>
	    ...
	    <%= comm.addDlistCheckHeading() %>
            ...
            <%= comm.addDlistColumnHeading((String)returnsNLS.get("itemCreditBeforeTax"),null,false,null,false ) %>
        <%= comm.endDlistRow() %>
	...
	%>
	    ...
	    <%
	    	BigDecimal creditAmountBeforeCharges = rmaItemLists[i].getCreditAmountInEJBType().add(rmaItemLists[i].getAdjustmentCreditInEJBType());
            ...
            <%= comm.addDlistColumn("<input type=\"text\" size=\"6\" name=\"creditAdjustment" + rmaItemLists[i].getRmaItemId() + "\"" + " value=\"" + rmaItemLists[i].getAdjustmentInEJBType() + "\"" + " onChange=\"setAnyChange();addToUpdate(" +rmaItemLists[i].getRmaItemId()+",\'creditAdjustment\');\"" + " >" , "none" ) %>
	    <SCRIPT>
		document.ReturnItemsListForm.creditAdjustment<%=rmaItemLists[i].getRmaItemId()%>.value=parent.parent.numberToCurrency(<%=rmaItemLists[i].getAdjustmentInEJBType()%>,"<%=rmaItemLists[i].getCurrency()%>","<%=jLanguageID%>");
	    </SCRIPT>
	    <%
	        BigDecimal creditBeforeTax = creditAmount.add(rmaItemLists[i].getAdjustmentInEJBType());
	        String strCreditBeforeTax = getFormattedAmount(creditBeforeTax, returnCurrencyBare, new Integer(jLanguageID), jStoreID);
	    %>
            <%= comm.addDlistColumn(strCreditBeforeTax, "none" ) %>
        <%= comm.endDlistRow() %>
		<SCRIPT>
