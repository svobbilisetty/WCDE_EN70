...
<%
  ...
  //String product_id = jsphelper.getParameter(ECConstants.EC_PRODUCT_NUMBER);
  String attr_id = jsphelper.getParameter(ECConstants.EC_ATTR_NUMBER);
  Integer logonLanguageId = cmdContext.getLanguageId();
  String  logonLanguage   = attrBean.getLanguageLocaleName(logonLanguageId);
%>
...
<SCRIPT>
  ...
  var isNewAttribute = <%=jsphelper.getParameter("isNewAttribute")%>;
  var languageBeforeChange = '<%=UIUtil.toJavaScript(logonLanguage)%>';
  ...
  var attributesOnEntry = new Array();
	...
	var checkObject = new Array();

<%
	Boolean bCatEntryIsMarkedForDelete = null;

	for (int iLang=0; iLang<supportedLanguages.length; iLang++)
	{
%>
		checkObject['<%=UIUtil.toJavaScript(attrBean.getLanguageLocaleName(supportedLanguages[iLang]))%>'] = new Object();
		checkObject['<%=UIUtil.toJavaScript(attrBean.getLanguageLocaleName(supportedLanguages[iLang]))%>'].values = new Array();
<%
		try 
		{
			AttributeValueAccessBean abValues = new AttributeValueAccessBean();
			Enumeration e1 = abValues.findByAttribute(new Long(attr_id), supportedLanguages[iLang]);
		
			while (e1.hasMoreElements())
			{
				abValues = (AttributeValueAccessBean) e1.nextElement();
				if (abValues.getCatalogEntryReferenceNumber().equals("0") == false)
				{

					CatalogEntryAccessBean abCatentry	 = new CatalogEntryAccessBean ();
					abCatentry.setInitKey_catalogEntryReferenceNumber( abValues.getCatalogEntryReferenceNumber() );
					abCatentry.getEJBRef();
					bCatEntryIsMarkedForDelete =  new Boolean (false);
					if (abCatentry.getMarkForDelete() != null)
					{
						if (abCatentry.getMarkForDeleteInEJBType().intValue() == 1)
						{
							bCatEntryIsMarkedForDelete = new Boolean (true);
						}
						else
						{
							bCatEntryIsMarkedForDelete = new Boolean (false);
						}
					}//End of if (abCatentry.getMarkForDelete() != null)

					if (! bCatEntryIsMarkedForDelete.booleanValue())
					{

	%>
						checkObject['<%=UIUtil.toJavaScript(attrBean.getLanguageLocaleName(supportedLanguages[iLang]))%>'].values["<%=UIUtil.toJavaScript(abValues.getAttributeValue().toString())%>"] = "true";
	<%
					}//End of if (! bCatEntryIsMarkedForDelete.booleanValue())
				}
			}
		} catch (Exception ex) {
		  	ECTrace.trace(ECTraceIdentifiers.COMPONENT_CATALOGTOOL, this.getClass().getName(), "main",
						"Exception retrieving catalog entry attribute value.");			  
		}
	}
...
%>
        ...
        attributes[supportedLanguages[i]].attributeValues[<%=j%>].original = '<%=UIUtil.toJavaScript(currDefaultLanguageProductAttributeValue.getAttributeValue().toString())%>';
        ...
        attributesOnEntry[supportedLanguages[i]].attributeValues[<%=j%>].original = '<%=UIUtil.toJavaScript(currDefaultLanguageProductAttributeValue.getAttributeValue().toString())%>';
...
<%
    ...
    while (allAttributesKeys.hasMoreElements()) {
      ...
      for ( int iLang=0 ; iLang < supportedLanguages.length ; iLang++) 
      {
      if (currProductAttribute.getLanguageId().equals(supportedLanguages[iLang]))
      {
...
%>
        updateAttributeValue(attributes,'<%=UIUtil.toJavaScript(currLocaleString)%>','<%=currProductAttributeValue.getAttributeValueId()%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getAttributeValue().toString())%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getDisplaySequence())%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getImage1())%>');
        updateAttributeValue(attributesOnEntry,'<%=UIUtil.toJavaScript(currLocaleString)%>','<%=currProductAttributeValue.getAttributeValueId()%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getAttributeValue().toString())%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getDisplaySequence())%>','<%=UIUtil.toJavaScript(currProductAttributeValue.getImage1())%>');
<%
      }
      }
      }
...
function updateAttributeValue(attributeArray,language,id,value,sequence,image) {
  ...
  attributeArray[language].attributeValues[i].original = value;	
...
function confirmValidImageInput(valueImageElementObj) {
  ...
  if (!top.isValidUTF8length(valueImageElementObj.value, 254)) {
...
function confirmValidValueInput(valueInputElementObj, type) {
  ...
  switch (type) {
    ...
    case "FLOAT" :
      ...
      break;
    case "STRING" :
      if (!top.isValidUTF8length(valueInputElementObj.value, 254)) {
        valueInputElementObj.select();
...
function formattedValue(value, languageId) {
  ...
  switch (attributeValueTypeRadioToString()) {
    case "INTEGER" :
     ...
     {
       return parent.numberToStr(value, languageId,0);
     }
    case "FLOAT" :
     if (value == null || value == undefined || value == '') 
     { 
     ...
     {
		return parent.numberToStr(value, languageId,null);
     }
    default:
     return value;   
  }
...
*/
        for (var i=0;i<attributes[languageSelect.value].attributeValues.length;i++) {
          ...
          insCell(tableName, dynamicListRowIndex, 1, newConvertFromTextToHTML(formattedValue(referenceValueFor(attributes[language].attributeValues[i].id), storeLanguageId)));
          insCell(tableName, dynamicListRowIndex, 2, newConvertFromTextToHTML(formattedValue(attributes[language].attributeValues[i].value, preferredLanguage)));
          insCell(tableName, dynamicListRowIndex, 3, newConvertFromTextToHTML(attributes[language].attributeValues[i].image));
...
///////////////////////////////////////////////////////////////////////////////////////////
//
// Modified by Jeff Shiner
// - changed to avoid the deletion of SKUs when attribute values are added/removed.
//   but does not allow the removal of a referenced attribute value.
//
///////////////////////////////////////////////////////////////////////////////////////////
	...
	for (var i=indicesToDelete.length-1;i>=0;i--) 
	{
		...
		for (language in attributes) 
		{
			...
			for (var j=attributes[language].attributeValues.length-1; j>=0; j--) 
			{
				...
				{
					if (checkObject[language].values[attributes[language].attributeValues[j].original] == "true") 
					{
						bFlag = false;
						alertDialog("Unable to delete value " + attributes[language].attributeValues[j].value + ".  This value is referenced by one or more SKUs.");
					} else {
...
function listInputModifyState() {
  ...
  valueInput.value=formattedValue(attributes[languageSelect.value].attributeValues[indexToModify].value,preferredLanguage);
...
function confirmValidDescriptionInput(attributeDescriptionElementObj) {
  ...
  if (!top.isValidUTF8length(attributeDescriptionElementObj.value, 254)) {
...
function confirmValidNameInput(attributeNameElementObj) {
  ...
  if (!top.isValidUTF8length(attributeNameElementObj.value, 254)) {
...
function confirmNewAttributeDeletingSKUsAcceptable() {
	...
	// modified by Jeff Shiner.   We no longer require the attributes to be deleted
	...
	for (var language in attributes) {
		if(attributes[language].name != null && attributes[language].name != undefined && attributes[language].name != '') {
			...
			if (bPassed == false)
			{
				...
				displayDataForLanguage(language);
				attributeName.select(); 
				alertDialog('<%=UIUtil.toJavaScript(attributeResource.get("atLeastNameAndOneValueMsg"))%>');    
				attributeName.focus();
				return false;
			}
		}
...
function visibleList(s){
   ...
   {
      this.languageSelect.style.visibility = s;
   }
}
...
<BODY CLASS="content" ONLOAD="displayDataForLanguage(languageSelect.value); parent.setContentFrameLoaded(true);" >
...
<LABEL for="languageSelect"><%=UIUtil.toHTML((String)attributeResource.get("language"))%></LABEL><BR>
<SELECT ID="languageSelect" NAME="languageSelect" ONCHANGE="languageSelectOnChange();">
<%
  ...
  for(int i=0;i<supportedLanguages.length;i++) {
    ...
    if (attrBean.getLanguageLocaleName(supportedLanguages[i]).equals(defaultLanguage))
    {
%>
    <OPTION VALUE='<%=UIUtil.toJavaScript(attrBean.getLanguageLocaleName(supportedLanguages[i]))%>' SELECTED><%=UIUtil.toHTML(currLanguage)%></OPTION>
<%
    } else {
%>
...
<LABEL for="attributeName"><%=UIUtil.toHTML((String)attributeResource.get("attributeNameTitle"))%></LABEL><BR>

<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0>
  <TR>
    <TD>
        <INPUT size="58" maxlength="254" type="text" id="attributeName" name="attributeName" ONBLUR="attributeNameOnBlur();">
    </TD>
    <TD>
...
<LABEL for="attributeDescription"><%=UIUtil.toHTML((String)attributeResource.get("description"))%></LABEL><BR>
<TEXTAREA id="attributeDescription" name="attributeDescription" value='' rows="5" cols="50" WRAP="HARD" ONBLUR="attributeDescriptionOnBlur();"></TEXTAREA><BR><BR>

<%=UIUtil.toHTML((String)attributeResource.get("type"))%><BR>

<INPUT TYPE="RADIO" ID="attributeValueTypeRadio1" NAME="attributeValueTypeRadio" ONCLICK="typeRadioButtonOnClick();" CHECKED><label for="attributeValueTypeRadio1"><%=UIUtil.toHTML((String)attributeResource.get("text"))%></label>
<INPUT TYPE="RADIO" ID="attributeValueTypeRadio2" NAME="attributeValueTypeRadio" ONCLICK="typeRadioButtonOnClick();"><label for="attributeValueTypeRadio2"><%=UIUtil.toHTML((String)attributeResource.get("wholeNumber"))%></label>
<INPUT TYPE="RADIO" ID="attributeValueTypeRadio3" NAME="attributeValueTypeRadio" ONCLICK="typeRadioButtonOnClick();"><label for="attributeValueTypeRadio3"><%=UIUtil.toHTML((String)attributeResource.get("decimalNumber"))%></label>
...
<SPAN ID="inputSpan" STYLE="display:none">
  <BR>
  <P>
  <TABLE class=dtable CELLSPACING=0 CELLPADDING=0 BORDER=0>
    <TR>
      <TD width=20% class=dtable STYLE="cursor:default; word-wrap:normal;">
        <LABEL for="valueInput"><%=UIUtil.toHTML((String)attributeResource.get("value"))%></LABEL>
      </TD>
      <TD width=20% class=dtable STYLE="cursor:default; word-wrap:normal;">
        <LABEL for="imageInput"><%=UIUtil.toHTML((String)attributeResource.get("image"))%></LABEL>
      </TD>
      <TD></TD>
    </TR>
    <TR>
      <TD>
        <INPUT TYPE="INPUT" ID="valueInput" NAME="valueInput"></INPUT>
      </TD>
      <TD>
        <INPUT TYPE="INPUT" ID="imageInput" NAME="imageInput"></INPUT>&nbsp;
      </TD>
      <TD>
        <SPAN ID="inputAddButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON" STYLE="width:auto" value="Ok" name="okButton1" ID="content" onClick="addInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("add"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputAddCancelButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON" STYLE="width:auto" value="Cancel" name="cancelAddButton" ID="content" onClick="cancelInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("cancelAdd"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputModifyButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON" STYLE="width:auto" value="Ok" name="okButton2" ID="content" onClick="modifyInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("change"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputModifyCancelButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON" STYLE="width:auto" value="Cancel" name="cancelModifyButton" ID="content" onClick="cancelInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("cancelChange"))%></BUTTON>
        </SPAN>
      </TD>
    </TR>
