...
var isSummary=<%=request.getParameter("isSummary")%>;
var myRef=<%=request.getParameter("refNum")%>;
var inputMethod=0;
...
if(isHostedModel) {
   ...
   document.writeln('<%=UIUtil.toJavaScript(pricingDataBeanList.getPricingDataBean(i).getProductName())%><BR>');
   document.writeln('<%=UIUtil.toJavaScript(pricingDataBeanList.getPricingDataBean(i).getProductSKU())%><BR>');
   document.writeln('</H1>')
   }
...
document.writeln('<%=UIUtil.toJavaScript(pricingDataBean.getProductSKU())%><BR>');
</SCRIPT>
</H1>
<P>
