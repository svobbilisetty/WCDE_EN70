...
<%
	...
	abCatalog.setInitKey_catalogReferenceNumber(strCatalogId);
	abCatalog.refreshCopyHelper();

%>

<TITLE><%=UIUtil.toHTML((String)rbCategory.get("NavCatCatalogUpdateContent_Title"))%></TITLE>
...
%>
	...
	function okButton() 
	{
		...
		outputObject.catalogId   = "<%=strCatalogId%>";
		...
		parent.catalogCreateButtons.submitFunction("NavCatCatalogUpdateControllerCmd", outputObject);
	} 
...
<BODY CLASS=content ONLOAD=onLoad() ONKEYPRESS=handleEnterPressed() ONCONTEXTMENU="return false;">
	...
	<H1><%=UIUtil.toHTML((String)rbCategory.get("NavCatCatalogUpdateContent_TitleText"))%></H1>

	<TABLE>   
		<TR><TD><LABEL for="CatalogCode"><%=UIUtil.toHTML((String)rbCategory.get("NavCatCatalogCreate_Code"))%></LABEL></TD></TR>
		<TR><TD><INPUT SIZE=32 MAXLENGTH=254 ID="CatalogCode" NAME=CatalogCode VALUE="<%=UIUtil.toHTML(abCatalog.getIdentifier())%>"></TD></TR>
		...
		<TR><TD><TEXTAREA ID="CatalogDesc" NAME=CatalogDesc ROWS=3 COLS=50 WRAP="HARD"><%=UIUtil.toHTML(abCatalog.getDescriptions())%></TEXTAREA></TD></TR>
		<TR HEIGHT=5><TD HEIGHT=5></TD></TR>
