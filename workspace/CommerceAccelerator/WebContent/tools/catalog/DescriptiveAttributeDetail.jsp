...
<%
	...
	Hashtable attributeResource = (Hashtable)com.ibm.commerce.tools.util.ResourceDirectory.lookup("catalog.AttributeNLS", cmdContext.getLocale());
...
<%
  ...
  String onlyOne  = jsphelper.getParameter("onlyOne");
...
<% 
  ...
  if(attr_id!=null) {
    ...
    attrBean.setProductId(new Long(product_id));
	 attrBean.setDescriptive(true);
...
function enableNone() {
	if (onlyOne == false || checkBoxes.length == 0) addButton.className='enabled';
	else                                            addButton.className='disabled';
...
function enableSingle() {    
	...
	if (onlyOne == false) addButton.className='enabled';
	else                  addButton.className='disabled';
...
function removePlaceHolders(attributeArray) {
  ...
  for(language in attributeArray) {
    ...
    for(var i=attributeArray[language].attributeValues.length-1;i>=0;i--) {
      if(attributeArray[language].attributeValues[i].value == null) {
      ...
      if(originalAttributeValue == null) {
        if (language == defaultLanguage)
        {
        	attributes[language].attributeValues[i].status = 'ADD';
                ...
                if (retrieveAttributeValue(attributesOnEntry,defaultLanguage,attributes[language].attributeValues[i].id) == null)
                {
        		attributes[language].attributeValues[i].status = 'ADD';
                } else {
...
<INPUT TYPE="RADIO" NAME="attributeValueTypeRadio" id="attributeValueTypeRadioId1" ONCLICK="typeRadioButtonOnClick();" CHECKED><label for="attributeValueTypeRadioId1"><%=UIUtil.toHTML((String)attributeResource.get("text"))%></label></INPUT>
<INPUT TYPE="RADIO" NAME="attributeValueTypeRadio" id="attributeValueTypeRadioId2" ONCLICK="typeRadioButtonOnClick();"><label for="attributeValueTypeRadioId2"><%=UIUtil.toHTML((String)attributeResource.get("wholeNumber"))%></label></INPUT>
<INPUT TYPE="RADIO" NAME="attributeValueTypeRadio" id="attributeValueTypeRadioId3" ONCLICK="typeRadioButtonOnClick();"><label for="attributeValueTypeRadioId3"><%=UIUtil.toHTML((String)attributeResource.get("decimalNumber"))%></label></INPUT>
...
<SPAN ID="inputSpan" STYLE="display:none">
  ...
  <TABLE>
    ...
    <TR>
      <TD>
        <INPUT TYPE="INPUT" width=100 ID="valueInput" NAME="valueInput"></INPUT>
      </TD>
      <TD>
        <INPUT TYPE="INPUT" width=100 ID="imageInput" NAME="imageInput"></INPUT>
      </TD>
      <TD>
        <SPAN ID="inputAddButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON"  STYLE="width:auto" value="Ok" name="okButton" ID="content" STYLE="border-width:1 1 1 1;text-align:center;" onClick="addInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("add"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputAddCancelButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON"  STYLE="width:auto" value="Cancel" name="cancelAddButton" ID="content" STYLE="border-width:1 1 1 1;text-align:center;" onClick="cancelInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("cancelAdd"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputModifyButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON"  STYLE="width:auto" value="Ok" name="okButton" ID="content" STYLE="border-width:1 1 1 1;text-align:center;" onClick="modifyInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("change"))%></BUTTON>
        </SPAN>

        <SPAN ID="inputModifyCancelButtonSpan" STYLE="display=none">
          <BUTTON type="BUTTON"  STYLE="width:auto" value="Cancel" name="cancelModifyButton" ID="content" STYLE="border-width:1 1 1 1;text-align:center;" onClick="cancelInputButtonAction();"><%=UIUtil.toHTML((String)attributeResource.get("cancelChange"))%></BUTTON>
        </SPAN>
      </TD>
    </TR>
...
<TABLE ID="layoutTable" cellpadding=0 cellspacing=0 border=0 width=100%>
