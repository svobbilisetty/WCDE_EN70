...
<TITLE><%=UIUtil.toHTML((String)rbCategory.get("NavCatSourceCreateCategory_Title"))%></TITLE>
...
<%
	for (int i=0; i<iLanguages.length; i++) 
	{
//		if (iLanguages[i].intValue() == cmdContext.getLanguageId().intValue() ) 
//		{
//%>
...
%>
	...
	//////////////////////////////////////////////////////////////////////////////////////
	// createButton(closeValue) 
	//
	// @param closeValue - true if we are to close the page after successful creation
	//
	// - this function validates the fields and submits the category for creation
	//////////////////////////////////////////////////////////////////////////////////////
	function createButton(closeValue) 
	{
		updateLanguageInfo();
		...
		outputObject.parentId     = parent.currentTargetTreeElement.id;
		outputObject.parentStoreId = parent.currentTargetTreeElement.STOREID;  
		outputObject.categoryCode = trim(CategoryCode.value);
		outputObject.languages    = vLanguages;
		
		closeAfterwards = closeValue;
		parent.workingFrame.submitFunction("NavCatCategoryCreateControllerCmd", outputObject);
	} 
	...
	// afterSuccess() 
	//
	// - this function is called after successful creation of a category
	//   It will close the page if the closeAfterwards value is set to true
	//////////////////////////////////////////////////////////////////////////////////////
	function afterSuccess() 
	{
		resetButton();
		if (closeAfterwards == true) parent.hideCreateCategory();
	}
	...
	// clearCategoryCode() 
	//
	// - this function clears the Category Code
	//////////////////////////////////////////////////////////////////////////////////////
	...
	function validateParameters()
	{
		...
		{
			alertDialog("<%=UIUtil.toJavaScript(rbCategory.get("NavCatSourceCreateCategory_ErrorNoParent"))%>");
			return false;
		}
	...
	function selectLanguage()
	{
		...
		{
			if (trim(CategoryCode.value) == "")
			{
				alertDialog("<%= UIUtil.toJavaScript((String)rbCategory.get("NavCatSourceCreateCategory_ErrorDefaultLangFirst"))%>"); 
	...
	// resetButton() 
	//
	// - this function clears all fields when the reset button is clicked
	//////////////////////////////////////////////////////////////////////////////////////
...
%>
			vLanguages[<%=i%>].name             = "";
			vLanguages[<%=i%>].shortDescription = "";
			vLanguages[<%=i%>].longDescription  = "";
			vLanguages[<%=i%>].thumbnail        = "";
			vLanguages[<%=i%>].fullimage        = "";
...
<BODY CLASS=content ONLOAD=onLoad() ONKEYPRESS=handleEnterPressed() >
	...
	<TABLE>   
		...
		<TR VALIGN=TOP><TD><INPUT SIZE=32 MAXLENGTH=254 ID="CategoryCode" NAME=CategoryCode VALUE=""></TD></TR>
