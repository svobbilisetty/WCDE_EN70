...
function validatePanelData(){
	...
	if (!isInputStringEmpty(form.minvalue.value) ) {
		var minvalue = form.minvalue.value;
		if (!isValidCurrency(minvalue,"<%= defaultCurrency %>","<%= lang %>")){
     			reprompt(form.minvalue, msgInvalidCurrency)
		     return false  
     		}
		if (minvalue.charAt(0) == '-'){
     		     reprompt(form.minvalue, msgNegativeNumber)
		     return false  
     		}
     }
     if (!isInputStringEmpty(form.minquant.value) ) {
 	      var minquant = form.minquant.value;
		var t= formatInteger(minquant,"<%= lang %>");
     		if(t==null || t=="NaN"){
		     reprompt(form.minquant, msgInvalidInteger)
		     return false  
	     }
	     if (minquant.charAt(0) == '-'){
     		     reprompt(form.minquant, msgNegativeNumber)
		     return false  
     	     }
     }
     return true;
...
function savePanelData(){
	...
	for (i=0;i<form.elements.length;i++) {
		...
		if (elem.name == "minvalue"){ 
			var primitive_val = "";
			if (!isInputStringEmpty(elem.value))
				primitive_val = currencyToNumber(elem.value,  "<%= defaultCurrency %>", "<%= lang %>");
			parent.put("minvalue",primitive_val);
			parent.put("minvalue_ds",elem.value);
		}
		else if (elem.name == "minquant") {
			var primitive_val = "";
			if (!isInputStringEmpty(elem.value))
				primitive_val = strToNumber(elem.value,"<%= lang %>");		
			parent.put("minquant",primitive_val);
...
function retrievePanelData(){
	...
	form.minvalue.value = parent.get("minvalue_ds","");
	form.minquant.value = parent.get("minquant_ds","");
...
<TABLE id="WC_W_BidruleRule_Table_1">
	<TR>
	<TD id="WC_W_BidruleRule_TableCell_1">
		<Label for="WC_W_BidruleRule_minvale_In_bidruleForm">
		<%= neg_properties.get("MinBidAmount") %></Label><BR>
      	<INPUT size="10" type="input" name="minvalue" maxlength="14" id="WC_W_BidruleRule_minvale_In_bidruleForm">
		
	</TD>
	</TR>

	<TR>
	<TD id="WC_W_BidruleRule_TableCell_2">
		<Label for="WC_W_BidruleRule_minquant_In_bidruleForm">
		<%= neg_properties.get("MinBidQty") %></Label><BR>	
      	<INPUT size="10" type="input" name="minquant" maxlength="9" id="WC_W_BidruleRule_minquant_In_bidruleForm">
...
%>
      ...
      <TR>
	<TD id="WC_W_BidruleRule_TableCell_3">
		<%= neg_properties.get("ValRange") %><BR>
		<Label for="WC_W_BidruleRule_ValRange1_In_bidruleForm">
		</Label>
		<INPUT TYPE="text" NAME="ValRange1" SIZE="10" MAXLENGTH="16" id="WC_W_BidruleRule_ValRange1_In_bidruleForm"><B>-</B>
		<Label for="WC_W_BidruleRule_ValRange2_In_bidruleForm">
		</Label>
		<INPUT TYPE="text" NAME="ValRange2" SIZE="10" MAXLENGTH="16" id="WC_W_BidruleRule_ValRange2_In_bidruleForm">
	      </TD>
	<TD id="WC_W_BidruleRule_TableCell_4">
		<Label for="WC_W_BidruleRule_ValIncr_In_bidruleForm">
		<%= neg_properties.get("ValIncr") %></Label><BR>
		<INPUT TYPE="text" NAME="ValIncr" SIZE=10 MAXLENGTH="16" id="WC_W_BidruleRule_ValIncr_In_bidruleForm">
      </TD>
      </TR>

       <TR>
	<TD id="WC_W_BidruleRule_TableCell_5">
	<Label for="WC_W_BidruleRule_ValueVector_In_bidruleForm"></Label>	
            <SELECT name="ValueVector"  size="5" width="75" onChange="onSelChangeValue()" id="WC_W_BidruleRule_ValueVector_In_bidruleForm">
            </SELECT>
	 </TD>

      <TD COLSPAN=6 id="WC_W_BidruleRule_TableCell_6">
            <INPUT TYPE=BUTTON NAME="AddButton" VALUE="<%= neg_properties.get("AddButton") %>" onClick="AddValueVector()" style="width:100" id="WC_W_BidruleRule_AddButton_In_bidruleForm"> <BR>
            <INPUT TYPE=BUTTON NAME="DeleteButton" VALUE="<%= neg_properties.get("remove") %>" onClick="DeleteValueVector()" style="width:100" id="WC_W_BidruleRule_DeleteButton_In_bidruleForm"> <BR>
            <!--INPUT TYPE=BUTTON NAME="ClearButton" VALUE="<%= neg_properties.get("ClearButton") %>" onClick="ClearValueVector()"--> <BR>
	</TD>
	</TR>
<% }
%>
