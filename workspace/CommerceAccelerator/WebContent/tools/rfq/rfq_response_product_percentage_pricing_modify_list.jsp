...
<script type="text/javascript">
    var productPPObjList;
    productObjList = parent.parent.get("<%= RFQConstants.EC_OFFERING_PERCENTPRICEPRODUCT  %>");
...
%>
        ...
        parent.parent.put("<%= RFQConstants.EC_OFFERING_PERCENTPRICEPRODUCT %>", productObjList);
    ...
    function product(reqProductId,reqCatentryid,reqPartNumber,reqProdType,reqName,reqProdDesc,reqCategoryName,reqPriceAdjustment,reqquantity,reqUnit,reqProdChangeable,productId,catentryid,partNumber,prodType,name,prodDesc,resPriceAdjustment,resQuantity,resQtyUnit,unitDesc,prodComment,prodAttribute) 
    {   
    ...
    function getCheckedProducts()  
    {
    	...
    	var form = document.rfqProdPPListForm;
    ...
    function ChangePriceAdjustment()   
    {
    	...
    	var url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseProductPercentagePricingRespondModify";
        if ( productObjList[index[0]].<%=RFQConstants.EC_RFQ_OFFERING_PRODUCT_CHANGEABLE%> == '<%=RFQConstants.EC_RFQ_PRODUCT_CHANGEABLE_YES%>') 
        {
    		var rfqId = '<%= RequestId %>';
    		url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseProductChangeablePercentagePricingRespondModify&amp;requestId=" + rfqId;
        }
    	top.saveModel(parent.parent.model);
    	top.saveData(productObjList,"allPercentagePricingProducts");
    ...
    {
        for (var i=0; i<document.rfqProdPPListForm.elements.length; i++)
        {
            var e = document.rfqProdPPListForm.elements[i];
            if (e.name != 'select_deselect')
            {
                e.checked = document.rfqProdPPListForm.select_deselect.checked;
            }
        }
        myRefreshButtons();
    }
    ...
    {
        document.rfqProdPPListForm.select_deselect.checked = false;
    }
...
<%= rfqNLS.get("instruction_Products_PercentagePrice_modify") %>
...
<form name="rfqProdPPListForm" action="">
<script type="text/javascript">
<%
...
%>	    
	    ...
	    addDlistColumn(ToHTML(resProdName));
            ...
            {
	    	addDlistColumn(ToHTML(resPriceAdjustment));	
            }
            else
            {
