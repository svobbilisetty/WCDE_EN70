...
<script type="text/javascript">
	...
	if (isFirstTimeLogonPanel6 == "0") 
	{  
	    //NOT first time log on panel8
	    ProductsArray = top.getData("allFixedPricingDynamicKits");
	} 
	else 
	{					
<%   
...
%>
		...
		} else 
		{
          	...
          	ProductsArray[<%=i%>].product_req_price=priceJS;         	         	
	  	ProductsArray[<%=i%>].product_req_currency="<%=prodCurrency%>";         
		...
		else 
		{
          	...
          	ProductsArray[<%=i%>].<%=RFQConstants.EC_OFFERING_PRICE%>= "<%=prodPrice%>";
          	ProductsArray[<%=i%>].product_res_price= priceJS;         
	  	...
	  	ProductsArray[<%=i%>].<%=RFQConstants.EC_OFFERING_CURRENCY%>= ProductsArray[<%=i%>].product_req_currency;  
...
%>
   	top.saveData(ProductsArray,"allFixedPricingDynamicKits");
   	}
    }
    ...
    {
        for (var i=0; i<document.rfqDynamickitFPListForm.elements.length; i++)
        {
            var e = document.rfqDynamickitFPListForm.elements[i];
            if (e.name != 'select_deselect')
            {
                e.checked = document.rfqDynamickitFPListForm.select_deselect.checked;
            }
        }
        myRefreshButtons();
    }
    ...
    {
        document.rfqDynamickitFPListForm.select_deselect.checked = false;
    }
...
%>
    ...
    { 
	parent.parent.put("fixPriceDynamicKit", ProductsArray );
	return true;
    }
	...
	var url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseDynamicKitFixedPricingRespond";
	if (anProduct.product_req_changeable == '<%=RFQConstants.EC_RFQ_PRODUCT_CHANGEABLE_YES%>') 
	{
	    var rfqId = '<%= jspHelper.getParameter("requestId")%>';
	    url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseDynamicKitChangeableFixedPricingRespond&amp;requestId=" + rfqId;
	}
	
	if (top.setReturningPanel) 
	{
	    top.setReturningPanel("rfqDynamicKitFixedPricing");
	} 	
	if (top.setContent) 
	{
	...
	for (var i=0; i<ProductsArray.length; i++) 
	{
		...
		if (ProductsArray[i].<%=RFQConstants.EC_RFQ_OFFERING_PRODUCTID%> == productID) 
		{
		    ...
		    ProductsArray[i].product_type=ProductsArray[i].product_req_type;
        	    ...
        	    ProductsArray[i].product_res_price= "";            	          	      
...
<%= rfqNLS.get("instruction_DynamicKits_FixedPrice") %>

<form name="rfqDynamickitFPListForm" method="get" action="">

<%= comm.startDlistTable((String)rfqNLS.get("rfqdynamickitfixedpricing")) %>
<%= comm.startDlistRowHeading() %>
<%= comm.addDlistCheckHeading(true,"selectDeselectAll()") %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqdynamickitname"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("resrequestprice"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("resrequestquantity"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqrequestunits"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqdynamickitcanbesubstituted"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqdynamickitsubstitutedinresponse"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqresponsedynamickitname"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("responseprice"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("responsequantity"),"null",false,"10%",wrap ) %>
<%= comm.addDlistColumnHeading((String)rfqNLS.get("rfqresponseunits"),"null",false,"10%",wrap ) %>
<%= comm.endDlistRow() %>
...
<script type="text/javascript">
	...
	addDlistColumn(ToHTML(ProductsArray[i].product_req_price));
	...
	addDlistColumn(ToHTML(ProductsArray[i].product_res_price));
