...
function goToAttachment(attachId) {
	var url = "RFQAttachmentView?<%= com.ibm.commerce.server.ECConstants.EC_ATTACH_ID %>=" + attachId + "&<%= com.ibm.commerce.rfq.utils.RFQConstants.EC_RFQ_REQUEST_ID %>=<%= rfqid %>";
...
function categorySummary(categoryId) {
	...
	var url = "DialogView?XMLFile=rfq.rfqAdjustmentCategorySummary&amp;categoryId="+categoryId+"&amp;rfqId=<%=rfqid%>";
...
<table>
    ...
    <tr>
		<td> <%= rfqNLS.get("rfqcustomerlogonid") %>: <a href="javascript:customerSummary(<%=rfqid%>)"><i><%= logonId %></i></a><br /></td>
    </tr>
    <tr>
