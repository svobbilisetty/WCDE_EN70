...
<script type="text/javascript">
    ...
    {
	isFirstTimeLogonPanel5="1";  //"1" means first time logon Panel3.
    ...
    {
	if (isFirstTimeLogonPanel5 == "0") 
	{   //NOT first time log on panel3
	    ProductsArray = top.getData("allFixedPricingProducts");
	} 
 	else 
	{					
<%   
		...
		else 
		{
	  	...
	  	if (ProductsArray[<%=i%>].<%=RFQConstants.EC_RFQ_OFFERING_CATENTRYID%> != null && !isEmpty(ProductsArray[<%=i%>].<%=RFQConstants.EC_RFQ_OFFERING_CATENTRYID%>)) 
	  	{
		    ...
		    ProductsArray[<%=i%>].product_name=ProductsArray[<%=i%>].product_req_name;
	  	    ProductsArray[<%=i%>].product_description="<%=prodDesc%>";	
		    ProductsArray[<%=i%>].product_type="<%=prodType%>";
		} 
		else 
		{
...
%>
   	top.saveData(ProductsArray,"allFixedPricingProducts");
   	}  
    }
    ...
    {
        for (var i=0; i<document.rfqProductFPListForm.elements.length; i++)
        {
            var e = document.rfqProductFPListForm.elements[i];
            if (e.name != 'select_deselect')
            {
                e.checked = document.rfqProductFPListForm.select_deselect.checked;
            }
        }
        myRefreshButtons();
    }
    ...
    {
        document.rfqProductFPListForm.select_deselect.checked = false;
    }
...
%>
    ...
    { 
	parent.parent.put("fixPriceProduct", ProductsArray );
	return true;
    }
	...
	var url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqresponseproductpriceqtrespond";
	if (anProduct.product_req_changeable == '<%=RFQConstants.EC_RFQ_PRODUCT_CHANGEABLE_YES%>') 
	{
	    var rfqId = '<%= jspHelper.getParameter("requestId")%>';
	    url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseProductChangeablePriceQtRespond&amp;requestId=" + rfqId;
	}
		    ...
		    ProductsArray[i].product_type=ProductsArray[i].product_req_type;
	  	    ...
	  	    if (ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%> != null)
	  	    {
		            ...
		            if (ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%>[k].req_userDefined == "Y")
			    {
				    ...
				    ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%>[k].<%=RFQConstants.EC_ATTR_UNIT%> = ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%>[k].req_unitId;
	  			    ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%>[k].<%=RFQConstants.EC_ATTR_OPERATOR%> = ProductsArray[i].<%=RFQConstants.EC_OFFERING_PRODATTRLIST%>[k].req_operatorId;
...
<%= rfqNLS.get("instruction_Products") %>

<form name="rfqProductFPListForm" method="get" action="">

<%= comm.startDlistTable((String)rfqNLS.get("rfqfixedpricing")) %>
...
<script type="text/javascript">
	...
	addDlistColumn(ProductsArray[i].product_req_price);			
	addDlistColumn(ProductsArray[i].product_req_quantity);		
	addDlistColumn(ProductsArray[i].product_req_unit);		
	addDlistColumn(prodChangeable);			
	addDlistColumn(prodSubstituted);		
	...
	addDlistColumn(ProductsArray[i].product_res_price);	
	addDlistColumn(ProductsArray[i].product_res_quantity);				
	addDlistColumn(ProductsArray[i].UnitDsc);
	endDlistRow(); 
