...
<%= fHeader%>
  ...
  <TITLE><%= nls.getString(PAStatsConstants.PE_BROWSER_TITLE)%></TITLE>
...
function getHelp () {
	...
	return "MC.productAdvisor.pestatistics.Help";
}   
...
function redirect() {
  ...
  var statsURL = "<%= PAStatsConstants.URL_PESTATS_VIEW %>";
  var storeId = "<%= storeId %>";	  
  ...
  if (top.setContent)      {
     ...
     top.setContent("<%= UIUtil.toJavaScript((String) nls.getString(PAStatsConstants.PE_BROWSER_TITLE) + " @ " + statisticsSource )%>", url, true);
  }    else     {
...
<jsp:useBean id="peStatsListBean" beanName="com.ibm.commerce.pa.stats.ProductExplorerStatisticsListDataBean" type="com.ibm.commerce.pa.stats.ProductExplorerStatisticsListDataBean" >
   <jsp:setProperty name="peStatsListBean" property="*"/>
   <% com.ibm.commerce.beans.DataBeanManager.activate( peStatsListBean, request); %>
</jsp:useBean>
...
if (  bRedirected  ||
	...
	try {
	  ...
	  com.ibm.commerce.pa.stats.ProductExplorerStatisticsBean peStatsBean;
	  com.ibm.commerce.pa.stats.ProductExplorerStatisticsBean[] peArray;
	
	
	  peArray = ( com.ibm.commerce.pa.stats.ProductExplorerStatisticsBean []) 
			peStatsListBean.getStatsDataBeanList ();
	...
	<TABLE CLASS=dtable align=center width='95%' cellpadding=0 cellspacing=0 border=0 width=100% bgcolor=#6D6D7C style="table-layout:fixed; border-bottom: 1px solid #6D6D7C; border-top: 0px solid #6D6D7C;border-right: 1px solid #6D6D7C;border-left: 1px solid #6D6D7C;">
	  ...
	  <CAPTION><h1><%= CampaignUtil.substitute((String) nls.getString ("peStatsTableCaption"), peStatsListBean.getStoreName ()) %></h1></CAPTION>
	  <TR class="dtableHeading">
	  		<TD align='left' nowrap class="COLHEADNORMAL"><%= nls.getString(PAStatsConstants.MSG_CATEGORY_ID_HEADING)%></TD>
    		<TD align='left' class="COLHEADNORMAL" nowrap><%= nls.getString(PAStatsConstants.MSG_CATEGORY_NAME_HEADING)%></TD>
			<TD align='middle' class="COLHEADNORMAL" nowrap><%= nls.getString(PAStatsConstants.MSG_FEATURE_NAME_HEADING)%></TD>
    		<TD align='left' nowrap class="COLHEADNORMAL"><%= nls.getString( PAStatsConstants.MSG_COUNT_HEADING ) %></TD>
	  </TR>
	...
	<%
	  ...
	  for (int j=0; j < peArray.length; j++) {
	     ...
	     peStatsBean = 
	           (com.ibm.commerce.pa.stats.ProductExplorerStatisticsBean ) peArray [j];
	 
	     countTotal += peStatsBean.getPeCountAsInteger().intValue();
	     
	%>
	
	  <TR CLASS=<%=classId%>>
	    <TD class="list_info1" ALIGN='left'> <%= peStatsBean.getCatGroupId ()%> </TD>
	    <TD class="list_info1" ALIGN='left'> <%= peStatsBean.getCatGroupName ()%> </TD>
	    <TD class="list_info1" ALIGN='left'> <%= peStatsBean.getFeatureName ()%> </TD>
	    <TD class="list_info1" ALIGN='left'> <%= peStatsBean.getPeCount ()%> </TD>
	  </TR>
	...
	<%
	  ...
	  <h1><%= nls.getString(PAStatsConstants.MSG_NO_DATA)%></h1>
	     ...
	     <%
	        ...
	        System.out.println (" PEstatistics: Exception :" + e.getMessage() );
	        e.printStackTrace();
	  }
