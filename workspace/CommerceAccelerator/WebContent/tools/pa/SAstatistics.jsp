...
<head>
   ...
   <TITLE><%= nls.getString(PAStatsConstants.SA_BROWSER_TITLE)%></TITLE>
...
function getHelp () {
	...
	return "MC.productAdvisor.sastatistics.Help";
}   
<%
...
function redirect() {
  ...
  var statsURL = "<%= PAStatsConstants.URL_SASTATS_VIEW %>";
  var storeId = "<%= storeId %>";
  ...
  if (top.setContent)      {
     ...
     top.setContent("<%= UIUtil.toJavaScript((String) nls.getString(PAStatsConstants.SA_BROWSER_TITLE) + " @ " + statisticsSource )%>", url, true);
  }    else     {
...
<jsp:useBean id="saStatsListBean" beanName="com.ibm.commerce.pa.stats.SalesAssistantStatisticsListDataBean" type="com.ibm.commerce.pa.stats.SalesAssistantStatisticsListDataBean" >
   <jsp:setProperty name="saStatsListBean" property="*"/>
   <% com.ibm.commerce.beans.DataBeanManager.activate( saStatsListBean, request); %>
</jsp:useBean>
...
if (  bRedirected  ||
	...
	try {
	  ...
	  com.ibm.commerce.pa.stats.SalesAssistantStatisticsBean saStatsBean;
	  com.ibm.commerce.pa.stats.SalesAssistantStatisticsBean[] saArray;
	
	
	  saArray = ( com.ibm.commerce.pa.stats.SalesAssistantStatisticsBean[]) 
			saStatsListBean.getStatsDataBeanList();
	...
	<TABLE CLASS=dtable align=center width='95%' cellpadding=0 cellspacing=0 border=0 width=100% bgcolor=#6D6D7C style="table-layout:fixed; border-bottom: 1px solid #6D6D7C; border-top: 0px solid #6D6D7C;border-right: 1px solid #6D6D7C;border-left: 1px solid #6D6D7C;">
	  ...
	  <CAPTION><h1><%= CampaignUtil.substitute((String) nls.getString ("saStatsTableCaption"), saStatsListBean.getStoreName ()) %></h1></CAPTION>
	   <TR class="dtableHeading">
	   		<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_CATEGORY_ID_HEADING)%></TH>
			<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_CATEGORY_NAME_HEADING)%></TD>
	    	<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_TYPE_HEADING)%></TD>
    		<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_TREE_ID_HEADING)%></TD>
    		<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_CONCEPT_ID_HEADING)%></TD>
    		<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_PARENT_CONCEPT_ID_HEADING)%></TD>    
    		<TD class="COLHEADNORMAL" align='left' nowrap><%= nls.getString(PAStatsConstants.MSG_CONCEPT_NAME_HEADING)%></TD>
	...
	<%
	  ...
	  String strAnswerConcept = nls.getString( PAStatsConstants.SA_ANSWER_CONCEPT );
	  int countTotal=0;
	  ...
	  for (int j=0; j < saArray.length; j++) {
	     ...
	     saStatsBean = 
	           (com.ibm.commerce.pa.stats.SalesAssistantStatisticsBean) saArray [j];
	 
	     countTotal += saStatsBean.getSaCountAsInteger().intValue();
	     
	     if ( saStatsBean.getConceptType ().equals( strAnswerConcept ) ) {
	...
	%>
	   ...
	   <TR CLASS=<%=classId%>>
	    <TD class="list_info1" ALIGN='left'> 	<%= saStatsBean.getCatGroupId ()%> </TD>
	    <TD class="list_info1" ALIGN='left'> 	<%= saStatsBean.getCatGroupName ()%> </TD>
	    <TD class="list_info1"  ALIGN='left'> 	<%= saStatsBean.getConceptType ()%> </TD>
	    <TD class="list_info1"  ALIGN='left'> 	<%= saStatsBean.getTreeId ()%> </TD>
	    <TD class="list_info1"  ALIGN='left'> 	<%= saStatsBean.getConceptId ()%> </TD>
	    <TD class="list_info1"  ALIGN='left'> 	<%= saStatsBean.getParentConceptId ()%> </TD>
	    <TD class="list_info1"  ALIGN=<%= nameAlign%>> <%= saStatsBean.getConceptName ()%> </TD>
	    <TD class="list_info1"  ALIGN='left'> 	<%= saStatsBean.getSaCount ()%> </TD>
	  </TR>
	...
	%>
	   ...
	   <TR class="list_row2">
	    <TD class="list_info1" ALIGN='right' COLSPAN='7'><%= nls.getString(PAStatsConstants.MSG_TOTAL_HEADING)%></TD>
	...
	<%
	  ...
	  <h1><%= nls.getString(PAStatsConstants.MSG_NO_DATA)%></h1>
	     ...
	     <%
	        ...
	        System.out.println (" SAstatistics: Exception :" + e.getMessage() );
	        e.printStackTrace();
	  }
