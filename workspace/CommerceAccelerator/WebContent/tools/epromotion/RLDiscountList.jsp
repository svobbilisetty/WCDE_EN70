...
%>
           ...
           function publish()
		   {
	        ...
	        if(originalCalcIDs != '')
			{
				top.showContent('/webapp/wcs/tools/servlet/RLPromotionPublish?calcodeId=' + originalCalcIDs + '&amp;RLPromotionDisplayLevel=2' + '&amp;status=1&amp;state=<%=state%>');
			}
           }
           function unpublish()
		   {
	        ...
	        if(originalCalcIDs != '')
			{
				top.showContent('/webapp/wcs/tools/servlet/RLPromotionPublish?calcodeId=' + originalCalcIDs + '&amp;RLPromotionDisplayLevel=2' + '&amp;status=0&amp;state=<%=state%>');
			}
           }
          ...
          function remove()
		  {
		        ...
		        if(originalCalcIDs != '')
			    {
				  if (parent.confirmDialog('<%= UIUtil.toJavaScript((String)NLSfile.get("DiscountDeleteMsg")) %>'))
                	top.showContent('/webapp/wcs/tools/servlet/RLPromotionDelete?calcodeId=' + originalCalcIDs + '&amp;RLPromotionDisplayLevel=2&amp;state=<%=state%>');
				}
	      }
...
%>
		 ...
		 <%= comm.startDlistRow(rowselect) %>
           ...
           %>
                 ...
                 <%= comm.addDlistColumn( tmpcol[1], "none", (String)NLSfile.get(tmp[1][0])) %>
           <%
           		String priority ="";
