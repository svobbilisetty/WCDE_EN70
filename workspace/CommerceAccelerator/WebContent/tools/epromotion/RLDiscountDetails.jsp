...
function writeCouponAndPromoCode()
{
	...
	else if(<%= rlDiscountDetails.getRLPromotion().getCodeRequired() %>)
	{
		...
		document.write('<i><%=rlDiscountDetails.getRLPromotion().getPromotionCode()%></i>');
	}
}
</script>
...
<i><%=rlDiscountDetails.getAdminDesc()%></i></p>
<%
}
%>
...
<i><%=rlDiscountDetails.getShopDesc()%></i></p>
<%
}
%>
...
<i><%=rlDiscountDetails.getShopLongDesc()%></i></p>
<%
}
%>
<%
