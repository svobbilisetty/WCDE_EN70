...
if (initiativeDataBean != null && initiativeDataBean.<%= CampaignConstants.ELEMENT_ID %> != "" && "<%= request.getParameter("newInitiative") %>" != "true") {
	newInitiative = false;
}
if (newInitiative) {
...
<%		for (int i=0; i<numberOfCampaigns; i++) { %>
	<option value="<%= campaignIdList[i] %>"><%= campaignNameList[i] %></option>
<%		} %>
</select>
<%	if (fromPanel.equals("campaign")) { %>
</div>
...
<i><%= emsName %>&nbsp;<%= campaignsRB.get("initiativeScheduleReadOnlyNotice") %></i>
