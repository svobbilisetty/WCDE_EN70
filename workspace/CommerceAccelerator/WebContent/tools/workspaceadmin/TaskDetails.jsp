...
<BODY CLASS=content ONLOAD=onLoad() ONCONTEXTMENU="return false;">
	...
	<%
		...
		try{
			...
			bQuickPublish=(abTaskGroup.getQuickPublishTypeInEJBType().intValue()==1);
			bPersistent=(abTaskGroup.getPersistentTypeInEJBType().intValue()==1);
	
			java.sql.Timestamp tsDueDate=abTaskGroup.getDueDateInEJBType();
			if(tsDueDate!=null)
			{
	...
	<%
		...
		try{
				...
				nTaskType=abTask.getTypeInEJBType();
				
				java.sql.Timestamp tsDueDate=abTask.getDueDateInEJBType();
				if(tsDueDate != null)
				{
					java.text.DateFormat df = java.text.DateFormat.getDateInstance(java.text.DateFormat.MEDIUM, locale);
