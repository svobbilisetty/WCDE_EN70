...
<form name="QueryTemplateForm" method="post">
  ...
  <tr>
   ...
   <td>	 	 
	  <textarea name="xpath" cols="100" rows="3" id="xpath"><%= (xpathStr == null)?(""):(xpathStr) %></textarea>
	 </td>
	</tr>
	<tr>
   <td VALIGN=TOP>
   ...
   <td>	
	  <textarea name=storeId cols="10" rows="1" id="storeid"><%= (storeId == null)?(""):(storeId) %></textarea>
	 </td>
  </tr>	
	<tr>
   <td VALIGN=TOP>	
   ...
   <td>	
	  <textarea  name=languageId cols="10" rows="1" id="languageid"><%= (languageId == null)?("-1"):(languageId) %></textarea><BR>
 	 </td>
  </tr>
 </table>
...
//		try {
		  ...
		  sbQuery.append("{") // access profile
		    ...
		    %>
		    <h3>Search Expression:</h3> <%=sbQuery.toString()%>
		    <% 
  ...
  stmt.close();
    ...
    if (exception != null) {
      ...
      %>
      	Search Expression: <br><%=sbQuery.toString()%><br><br>
      <%} %>
      <% 
...
function loadPanelData() {
   ...
   // component list
    for (var i=0; i<componentlist.length; i++) {
      if (componentlist.options[i].value == "<%= componentId %>") {
         ...
         for (var j=0; j<xpathnameslist.length; j++) {
            if (xpathnameslist.options[j].value == "<%= xpathName %>") {
               xpathnameslist.selectedIndex = j;
               updateXPathAndAccessProfile(j);
               document.getElementById("xpath").innerHTML = "<%= xpathStr %>";
               
               // access profile list
               for (var k=0; k<accessprofilelist.length; k++) {
                   if (accessprofilelist.options[k].value == "<%= accessProfile %>") {
