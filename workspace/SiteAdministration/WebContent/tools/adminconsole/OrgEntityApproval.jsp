...
Hashtable userEntityNLS = (Hashtable)com.ibm.commerce.tools.util.ResourceDirectory.lookup("adminconsole.AdminConsoleNLS", locale);
   if (orgEntityNLS == null) System.out.println("!!!! RS is null");
	...
	MemberGroupTypeDataBean[] mgtdb = null;
	MemberGroupTypeDataBean[] mgtdb2 = null;
...
<%     approvalBean.populate();   
       ...
       mgtdb = approvalBean.getMemberGroupTypeDataBeans();    
       } catch (Exception e) {}
       
        java.text.Collator collator = java.text.Collator.getInstance(locale);
	MemberGroupTypeDataBean tmp = null;
	for (int i = 0; i < mgtdb.length; i++) {
			for (int j = i + 1; j < mgtdb.length; j++) {
				if (collator.compare(mgtdb[i].getName(), mgtdb[j].getName()) > 0) {
					tmp = mgtdb[i];
					mgtdb[i] = mgtdb[j];
					mgtdb[j] = tmp;
			}
       ...
       mgtdb2 = new MemberGroupTypeDataBean[mgtdb.length];
       int index =0;
       for (int j=0; j < mgtdb.length; j++) {
	        try{
	                MemberGroupAccessBean mgab = new MemberGroupAccessBean();
		        MemberGroupAccessBean temp = mgab.findByOwnerName(new Long (strOrgEntityId), mgtdb[j].getName());
		        mgtdb2[index] = mgtdb[j];
		        index++;
...
function initializeState()
{
  ...
  for (int i =0; i < mgtdb.length; i++) {
       ...
       for (int k = 0; k < mgtdb2.length; k++) {
            try{
            String impRoles = getImplicitRoles(mgtdb[i].getProperties());
			          if (impRoles == null) 
            				impRoles = "";
            if ((mgtdb2[k].getName()).equals(mgtdb[i].getName())) {
                out.println("OrgAppArray[" + k + "] = new Array();");
            	out.println("OrgAppArray[" + k + "].id ='" + mgtdb[i].getMbrGrpTypeId() + "';");
		out.println("OrgAppArray[" + k + "].name ='" + mgtdb[i].getName() + "';");
		out.println("OrgAppArray[" + k + "].description ='" + mgtdb[i].getDescription() + "';");
       ...
       String impRoles2 = getImplicitRoles(mgtdb[i].getProperties());
       if (impRoles2 == null) 
       		impRoles2 = "";
  
       out.println("AppArray[" + (i - offset) + "] = new Array();");
       out.println("AppArray[" + (i - offset)+ "].name ='" + mgtdb[i].getName() + "';");
       out.println("AppArray[" + (i - offset)+ "].description ='" + mgtdb[i].getDescription() + "';");
       out.println("AppArray[" + (i - offset)+ "].id ='" + mgtdb[i].getMbrGrpTypeId() + "';");
...
function savePanelData()
{
    ...
    parent.put("<%=ECUserConstants.EC_ROLE_ORGENTITYID%>","<%=strOrgEntityId%>");
...
<TABLE BORDER='0' summary="<%=UIUtil.toHTML((String)orgEntityNLS.get("userGeneralRoles"))%>">
    ...
    <TD WIDTH='20'>&nbsp;</TD>
    <TD>
	<LABEL for="avrolesList1b"><%=UIUtil.toHTML((String)userEntityNLS.get("orgGeneralAvailableApp"))%></LABEL>
    </TD>
    </TR>
