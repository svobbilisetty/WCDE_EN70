...
%>
     ...
     function changeCriteria() {
         ...
         var url = top.getWebappPath() + "DialogView?ActionXMLFile=adminconsole.StoreSchedulerMain&amp;XMLFile=adminconsole.SchedulerCriteria&amp;cmd=StoreSchedulerMainView&amp;listsize=<%=listSize%>&amp;startindex=<%=startIndex%>&amp;orderby=<%=orderByParm%>&amp;startDateCriteria=<%=startTime%>&amp;endDateCriteria=<%=endTime%>&amp;refnum=0'";
         parent.location.replace(url);
...
<%= addHiddenVars() %>
<INPUT TYPE="HIDDEN" NAME="startDateCriteria" VALUE="<%= (startTime != null) ? startTime : "" %>">
<INPUT TYPE="HIDDEN" NAME="endDateCriteria" VALUE="<%= (endTime != null) ? endTime : "" %>">
