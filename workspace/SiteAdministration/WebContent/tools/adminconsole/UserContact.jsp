...
function initializeState()
{
   ...
   if ("<%=memberId2%>" != null || "<%=memberId2%>" != '') {
      
      document.contact.email1.value  = "<%=userBean.getEmail1()%>";
      document.contact.email2.value    = "<%=userBean.getEmail2()%>";
      document.contact.phone1.value    = "<%=userBean.getPhone1()%>";
      document.contact.phone2.value   = "<%=userBean.getPhone2()%>";
      document.contact.fax1.value  = "<%=userBean.getFax1()%>";
      document.contact.fax2.value  = "<%=userBean.getFax2()%>";
      
      
      for ( var i=0; i < document.contact.preferredCommunication.length ; i++ )
      {
	      if ( document.contact.preferredCommunication.options[i].value == "<%=userBean.getPreferredCommunication()%>")
         {
      ...
      %>
         ...
         {
           if ( document.contact.phone1Type.options[i].value == "<%=userBean.getPhone1Type()%>")
           {
         ...
         {
            if ( document.contact.phone2Type.options[i].value == "<%=userBean.getPhone2Type()%>")
            {
      ...
      {
         if (document.contact.bestTimeToCall.options[i].value == "<%=userBean.getBestCallingTime()%>")
         {
...
<%          }
            if (showPhoneListed.equals("true")) {
%>           <TD valign="bottom">
              <INPUT type="checkbox" name="phone1Listed" id="phone1Listed1" value="1">
              <LABEL for="phone1Listed1"><%=userNLS.get("listed")%></LABEL></TD>
<%          }
%>        </TR>
          <TR>
            <TD valign="bottom">
...
<%          }
            if (showPhoneListed.equals("true")) {
%>            <TD valign="bottom">
              <INPUT type="checkbox" name="phone2Listed" id="phone2Listed1" value="1">
              <LABEL for="phone2Listed1"><%=userNLS.get("listed")%></LABEL></TD>
<%          }
%>        </TR>
          ...
          <TR>
            ...
            <LABEL for="bestTimeToCall_1"><%=userNLS.get("bestTimeToCall")%></LABEL><BR>
            <SELECT name="bestTimeToCall" id="bestTimeToCall_1">
            <OPTION value="" selected><%=userNLS.get("notProvided")%></OPTION>
