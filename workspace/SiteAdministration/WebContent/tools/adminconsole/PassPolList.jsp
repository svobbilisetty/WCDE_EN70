...
PolicyPasswordDataBean ppab = new PolicyPasswordDataBean();
//DataBeanManager.activate(ppab, request);
ppab.setCommandContext(cmdContext);

Vector passPolList = ppab.getPolicyPassword();

int numberOfPassPol = passPolList.size();
...
<TITLE><%= UIUtil.toHTML((String)securityNLS.get("passPolicy")) %></TITLE>

<SCRIPT LANGUAGE="JavaScript">
<!---- hide script from old browsers

function getResultsSize() { 
     return <%= numberOfPassPol  %>; 
}

    function newRole()
    {
      var url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.PassPolDialog";
      if (top.setContent)
      {
        top.setContent("<%= UIUtil.toJavaScript((String)securityNLS.get("newPassPolicy")) %>",
                       url,
                       true);
    ...
    function changeRole()
    {
      ...
      if (changed != 0)
      {
        var url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.PassPolEditDialog";
        url += "&passId=" + passId;
        if (top.setContent)
        {
          top.setContent("<%= UIUtil.toJavaScript((String)securityNLS.get("changePassPolicy")) %>",
                         url,
                         true);
        }
        else
...
function deleteRole() {
	...
	<% for (int i=0; i < accPolList.size(); i++) {
		...
		String alpId = (String) temp.elementAt(5);
		String name = (String) temp.elementAt(4);
	%>
	...
	var redirectURL = top.getWebappPath() + "NewDynamicListView?ActionXMLFile=adminconsole.PassPolList&amp;cmd=PassPolListView";
          var url = top.getWebappPath() + "PasswordPolicyDelete?"
                    + "plcypasswd_id=" + deleteId
...
<%
	  ...
	  int endIndex = getStartIndex() + getListSize();
    	  if (endIndex > numberOfPassPol)
      		endIndex = numberOfPassPol;
          int totalsize = numberOfPassPol;
          int totalpage = totalsize/listSize;          
	
%>
<%=com.ibm.commerce.tools.common.ui.taglibs.comm.addControlPanel("adminconsole.PassPolList", totalpage, totalsize, cmdContext.getLocale() )%>

<FORM NAME="accPolForm" ACTION="PassPolListView" METHOD="POST">
<%=addHiddenVars()%>

<%= com.ibm.commerce.tools.common.ui.taglibs.comm.startDlistTable((String)securityNLS.get("passPolicy")) %>
...
<%
    ...
    {
      	Vector temp = (Vector) passPolList.elementAt(i);
   	String id = (String) temp.elementAt(0);
   	String description = (String) temp.elementAt(9);
