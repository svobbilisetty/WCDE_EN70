...
function getResultsSize() { 
  ...
  function activate_deactivateStoreTransport() {
      ...
      // build URL string
        ...
        var url = top.getWebappPath() + "StoreTransUpdate?"
        var params = "URL=" + top.getWebappPath() + "NewDynamicListView?ActionXMLFile=adminconsole.MsgStoreTransport&amp;cmd=MsgStoreTransportView&amp;listsize=20&amp;startindex=0&amp;refnum=0&amp;<%=MessagingSystemConstants.TC_STORETRANS_STORE_ID%>=<%=store_id%>";

        var deleteParm = "<%=MessagingSystemConstants.TC_STORETRANS_TRANSPORT_ID%>=";
        var i = 0;
        if (checked.length > 0) {
