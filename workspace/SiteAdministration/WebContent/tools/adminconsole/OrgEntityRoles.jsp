...
<%
     ...
     Hashtable orgEntityNLS = (Hashtable)com.ibm.commerce.tools.util.ResourceDirectory.lookup("adminconsole.OrgEntityNLS", locale);
      ...
      OrganizationDataBean odb = new OrganizationDataBean();
      odb.setDataBeanKeyMemberId(orgId);
      com.ibm.commerce.beans.DataBeanManager.activate(odb, request);
      orgRoles = odb.getRoles();
      String parentId = odb.getParentMemberId();
...
{  
  ...
  orarray = parent.get("OrgRoleArray");
    ...
    parent.put("<%=ECUserConstants.EC_ROLE_ORGENTITYID%>","<%=orgId%>");
    ...
    for (var i=0; i < document.f1.selrolesList.length; i++) {
    	...
    	parent.put("<%=ECUserConstants.EC_ROLE_ORGENTITYID%>"+x,"<%=orgId%>");
    }
