...
<% } %>
  ...
  function deleteMsg() {
      ...
      if ( confirmDialog('<%= UIUtil.toJavaScript((String)messagingListNLS.get("messagingMsgViewDeleteQuestion")) %>') ) {
        ...
        <% if(table.intValue()==0) { %>
           var params = "URL=" + top.getWebappPath() + "NewDynamicListView?ActionXMLFile=adminconsole.MsgArchiveListFromFind&amp;cmd=AwaitingMsgStoreDisplayFilterView&amp;listsize=20&amp;startindex=0&amp;refnum=0&amp;delivery=<%=request.getParameter("delivery")%>&amp;<%=MessagingSystemConstants.MSG_VIEW_TRANSPORT_ID%>=<%=msgViewList.getTransportId()%>&amp;<%=MessagingSystemConstants.MSG_VIEW_TABLE%>=<%=table.toString()%>&amp;<%=MessagingSystemConstants.MSG_VIEW_PROFILE_ID%>=-1";
        <% } else if(table.intValue()==1) { %>
           var params = "URL=" + top.getWebappPath() + "NewDynamicListView?ActionXMLFile=adminconsole.MsgStoreListFromFind&amp;cmd=AwaitingMsgStoreDisplayFilterView&amp;listsize=20&amp;startindex=0&amp;refnum=0&amp;delivery=<%=request.getParameter("delivery")%>&amp;<%=MessagingSystemConstants.MSG_VIEW_TRANSPORT_ID%>=<%=msgViewList.getTransportId()%>&amp;<%=MessagingSystemConstants.MSG_VIEW_TABLE%>=<%=table.toString()%>&amp;<%=MessagingSystemConstants.MSG_VIEW_PROFILE_ID%>=-1";
        <% } %>
