  ...
  top.showProgressIndicator(false);
   ...
   if (LUS_isOrgsDataFrameInitialized==false)
      {
         ...
         if (resultCondition=='1')
         {
			...
			for(var j=0; j<document.param[memberName].options.length;j++){
				if(document.param[memberName].options[j].value == defaultValue){
					document.param[memberName].options[j].selected = true;
					document.param.LUS_QuickTextEntry.value = document.param[memberName].options[j].text;
				}
			}
         }
   ...
   { 
      ...
      // and update the currently showing status line.
		...
		document.param.LUS_QuickTextEntry.value = "";
		for(var j=0; j<document.param[memberName].options.length;j++){
				if(document.param[memberName].options[j].value == defaultValue){
					document.param[memberName].options[j].selected = true;
					document.param.LUS_QuickTextEntry.value = document.param[memberName].options[j].text;
...
function LUS_SelectResultItem()
{
   ...
   document.param.LUS_QuickTextEntry.value = text[0];
}
...
function validateParentOrg()
{
	if (document.param[memberName].selectedIndex < 0) {
		return false;
	} else {
		return true;
	}
