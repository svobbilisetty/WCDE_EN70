...
var dataFrame = '<iframe name="dframe" title="<%= dataFrameTitle %>" id="dataframe" width="0" height="0" src="' + webappPath + 'DynamicTreeFetchDataView?<%= tree.getInitDataURLParam() %><%= UIUtil.toJavaScript(NVPs.toString()) %><%if (request.getParameter("gotoNode")!=null) { %>&gotoNode=<%=(String)request.getParameter("gotoNode")%><%}%>"></iframe>';
