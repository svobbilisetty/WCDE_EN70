...
<jsp:useBean id="actionGroupForPolicyBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ActionGroupLightListBeanUtility" >
<jsp:setProperty property="languageId" name="actionGroupForPolicyBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      actionGroupForPolicyBean.setSortAtt( sort );
     
  String policyId = (String) request.getParameter("policyId");
   actionGroupForPolicyBean.setPolicyId(Integer.valueOf(policyId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(actionGroupForPolicyBean, request);
   ActionGroupLightDataBean[] aList = actionGroupForPolicyBean.getActionGroupBeans();
%>
...
function getResultsSize() { 
     return <%= actionGroupForPolicyBean.getActionGroupBeans().length %>; 
}
...
{
  for (var i=0; i < document.ActionGroupForPolicyViewForm.elements.length; i++) 
  {
     var e = document.ActionGroupForPolicyViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = actionGroupForPolicyBean.getActionGroupBeans().length;
...
<%= comm.addControlPanel("policyeditor.ActionGroupForPolicyView",totalpage,totalsize,locale) %>
<FORM NAME="ActionGroupForPolicyViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyActionGroupForPolicyView"))); 
   }
%>
