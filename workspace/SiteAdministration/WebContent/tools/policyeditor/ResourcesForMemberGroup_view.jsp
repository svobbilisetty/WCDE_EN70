...
<jsp:useBean id="resourcesForMemberGroupBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ResourceLightListBeanUtility" >
<jsp:setProperty property="languageId" name="resourcesForMemberGroupBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
  ...
  if ( sortby != null && !sortby.equals("null") && !sortby.equals("") ) 
      ...
      resourcesForMemberGroupBean.setSortAtt( sort );

     
  String memberGroupId= (String) request.getParameter("memberGroupId");
   resourcesForMemberGroupBean.setMemberGroupId(Long.valueOf(memberGroupId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(resourcesForMemberGroupBean, request);
   ResourceLightDataBean[] aList = resourcesForMemberGroupBean.getResourceBeans();
%>
...
function getResultsSize() { 
     return <%= resourcesForMemberGroupBean.getResourceBeans().length %>; 
}
...
{
  for (var i=0; i < document.ResourcesForMemberGroupViewForm.elements.length; i++) 
  {
     var e = document.ResourcesForMemberGroupViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = resourcesForMemberGroupBean.getResourceBeans().length;
...
<%= comm.addControlPanel("policyeditor.ResourcesForMemberGroupView",totalpage,totalsize,locale) %>
<FORM NAME="ResourcesForMemberGroupViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyResourcesForMemberGroupView"))); 
   }
%>
