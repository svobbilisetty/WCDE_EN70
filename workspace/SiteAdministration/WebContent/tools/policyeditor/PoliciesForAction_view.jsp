...
<jsp:useBean id="policiesForActionBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.PolicyLightListBeanUtility" >
<jsp:setProperty property="languageId" name="policiesForActionBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
     ...
     if ( sortby != null && !sortby.equals("null") && !sortby.equals("") ) 
      ...
      policiesForActionBean.setSortAtt( sort );
     
      String actionId = (String) request.getParameter("actionId");
   policiesForActionBean.setActionId(Integer.valueOf(actionId)) ;
 com.ibm.commerce.beans.DataBeanManager.activate(policiesForActionBean, request);
   PolicyLightDataBean[] aList = policiesForActionBean.getPolicyBeans();
%>
...
function getResultsSize() { 
     return <%= policiesForActionBean.getPolicyBeans().length %>; 
}
...
{
  for (var i=0; i < document.PoliciesForActionViewForm.elements.length; i++) 
  {
     var e = document.PoliciesForActionViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = policiesForActionBean.getPolicyBeans().length;
...
<%= comm.addControlPanel("policyeditor.PoliciesForActionView",totalpage,totalsize,locale) %>
<FORM NAME="PoliciesForActionViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyPoliciesForActionView"))); 
   }
%>
