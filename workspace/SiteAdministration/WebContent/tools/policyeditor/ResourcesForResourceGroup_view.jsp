...
<jsp:useBean id="resourcesForResourceGroupBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ResourceLightListBeanUtility" >
<jsp:setProperty property="languageId" name="resourcesForResourceGroupBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
  ...
  PolicySortingAttribute sort = new PolicySortingAttribute();
      ...
      resourcesForResourceGroupBean.setSortAtt( sort );
     
  String resourceGroupId = (String) request.getParameter("resourceGroupId");
   resourcesForResourceGroupBean.setResourceGroupId(Integer.valueOf(resourceGroupId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(resourcesForResourceGroupBean, request);
   ResourceLightDataBean[] aList = resourcesForResourceGroupBean.getResourceBeans();
%>
...
function getResultsSize() { 
     return <%= resourcesForResourceGroupBean.getResourceBeans().length %>; 
}
...
{
  for (var i=0; i < document.ResourcesForResourceGroupViewForm.elements.length; i++) 
  {
     var e = document.ResourcesForResourceGroupViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = resourcesForResourceGroupBean.getResourceBeans().length;
...
<%= comm.addControlPanel("policyeditor.ResourcesForResourceGroupView",totalpage,totalsize,locale) %>
<FORM NAME="ResourcesForResourceGroupViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyResourcesForResourceGroupView"))); 
   }
%>
