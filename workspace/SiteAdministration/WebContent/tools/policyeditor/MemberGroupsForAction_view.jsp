...
<jsp:useBean id="memberGroupForActionBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.MemberGroupLightListBeanUtility" >
<jsp:setProperty property="languageId" name="memberGroupForActionBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      memberGroupForActionBean.setSortAtt( sort );
     
  String actionId= (String) request.getParameter("actionId");
   memberGroupForActionBean.setActionId(Integer.valueOf(actionId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(memberGroupForActionBean, request);
   MemberGroupLightDataBean[] aList = memberGroupForActionBean.getMemberGroupBeans();
%>
...
function getResultsSize() { 
     return <%= memberGroupForActionBean.getMemberGroupBeans().length %>; 
}
...
{
  for (var i=0; i < document.MemberGroupsForActionViewForm.elements.length; i++) 
  {
     var e = document.MemberGroupsForActionViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = memberGroupForActionBean.getMemberGroupBeans().length;
...
<%= comm.addControlPanel("policyeditor.MemberGroupsForActionView",totalpage,totalsize,locale) %>
<FORM NAME="MemberGroupsForActionViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyMemberGroupsForActionView"))); 
   }
%>
