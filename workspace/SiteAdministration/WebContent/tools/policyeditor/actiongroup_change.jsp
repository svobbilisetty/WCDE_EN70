...
<%        
   ...
   ActGrpDescDataBean ActGrpDescDataBean = new ActGrpDescDataBean();
   ActGrpDescDataBean.setCommandContext(aCommandContext);
   com.ibm.commerce.beans.DataBeanManager.activate(ActGrpDescDataBean, request);

   ActGrpDataBean ActGrpDataBean = new ActGrpDataBean();
   ActGrpDataBean.setCommandContext(aCommandContext);
   com.ibm.commerce.beans.DataBeanManager.activate(ActGrpDataBean, request);

   String untrimmedDesc = ActGrpDescDataBean.getDescription();
   if (untrimmedDesc != null) 
   {
	   description = untrimmedDesc.trim();
   }

   String untrimmeddisplayName = ActGrpDescDataBean.getDisplayName();
   if (untrimmeddisplayName != null) 
   {
	   displayName = untrimmeddisplayName.trim();
   }

   String untrimmedactGrpName = ActGrpDataBean.getActGrpName();
   if (untrimmedactGrpName != null) 
   {
	   actGrpName = untrimmedactGrpName.trim();
   }

//   String displayName = ActGrpDescDataBean.getBaseName().trim();
//   String actGrpDisplayName = ActGrpDescDataBean.getDisplayName().trim();
   ...
   actGrpId = (String) request.getParameter("actGrpId");
...
<TITLE><%= UIUtil.toHTML((String)policyNLS.get("changeActionGroup")) %></TITLE>
...
<%   
   ...
   actionListBean.setActionGroupId(Integer.valueOf(actGrpId));   
   com.ibm.commerce.beans.DataBeanManager.activate(actionListBean, request);   
   ActionLightDataBean[] aList = actionListBean.getActionBeansForActGrp();
   ActionLightDataBean[] bList = actionListBean.getActionBeansNotInActGrp();
%>
...
<H1><%= UIUtil.toHTML((String)policyNLS.get("changeActionGroup")) %></H1>
...
<TABLE>
  ...
  <TR>
    <TD><INPUT size="54" maxlength="128" name="actiongroupName" value="<%=UIUtil.toHTML(actGrpName)%>" id="actiongroupNameHeader"></TD></TR>
  <TR>
    <TD ALIGN="LEFT"><STRONG><Label for="displayNameHeader"><%= UIUtil.toHTML((String)policyNLS.get("displayNameHeader")) %></Label></STRONG> </TD></TR>
  <TR>
    <TD><INPUT size="54" maxlength="128" name="actGrpDisplayName" value="<%=UIUtil.toHTML(displayName)%>" id="displayNameHeader"></TD></TR>
  <TR>
  ...
  <TR>
    <TD><INPUT size=54 maxlength="254" name="actiongroupDescription" value="<%=UIUtil.toHTML(description)%>" id="actiongroupDescriptionHeader"></TD></TR>
  <TR>
...
<TR>
   <TD ALIGN="LEFT"><LABEL for="actionAvailable1"><STRONG><%= UIUtil.toHTML((String)policyNLS.get("actionAvailableHeader")) %></LABEL></STRONG></TD>
  </TR><TR>
    <TD VALIGN="BOTTOM" CLASS="selectWidth">
...
<%
     ...
     {
%>
     <OPTION value="<%= bList[j].getActionId().toString() %>"><%= UIUtil.toHTML(bList[j].getActionBaseName()) %></OPTION>
<%
     }
%>
</SELECT></TD>
    ...
    <INPUT TYPE="HIDDEN" NAME="actionGroupId" VALUE="<%= actGrpId %>">
    <INPUT TYPE="HIDDEN" NAME="viewtaskname" VALUE="<%= viewtaskname %>">
