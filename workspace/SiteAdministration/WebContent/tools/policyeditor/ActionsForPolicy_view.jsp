...
<jsp:useBean id="actionsForPolicyBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ActionLightListBeanUtility" >
<jsp:setProperty property="languageId" name="actionsForPolicyBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      actionsForPolicyBean.setSortAtt( sort );
     
  String policy_id = (String) request.getParameter("policyId");
   actionsForPolicyBean.setPolicyId(Integer.valueOf(policy_id)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(actionsForPolicyBean, request);
   ActionLightDataBean[] aList = actionsForPolicyBean.getActionBeans();
%>
...
function getResultsSize() { 
     return <%= actionsForPolicyBean.getActionBeans().length %>; 
}
...
{
  for (var i=0; i < document.ActionsForPolicyViewForm.elements.length; i++) 
  {
     var e = document.ActionsForPolicyViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = actionsForPolicyBean.getActionBeans().length;
...
<%= comm.addControlPanel("policyeditor.ActionsForPolicyView",totalpage,totalsize,locale) %>
<FORM NAME="ActionsForPolicyViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyActionsForPolicyView"))); 
   }
%>
