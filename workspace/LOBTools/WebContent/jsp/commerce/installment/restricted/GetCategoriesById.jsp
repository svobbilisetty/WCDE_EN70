...
<c:forEach var="catalogGroup" items="${categories}">
	...
	<object objectType="${category.associationType}${referenceObjectType}">
		...
		<c:choose>
			<c:when test="${type == 'SalesCatalogGroup'}">
				<jsp:directive.include file="/jsp/commerce/catalog/restricted/serialize/SerializeSalesCatalogGroup.jspf" />
			</c:when>
			<c:otherwise>
