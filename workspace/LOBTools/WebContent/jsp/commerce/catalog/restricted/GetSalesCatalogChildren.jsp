...
<objects
	...
	<c:forEach var="catalogGroup" items="${topCategories}">
		...
		<c:set var="childType" value="${shared}${inherited}${type}ForSalesCatalog"/>
		...
		<c:if test="${(inherited == 'Inherited') && (param.catalogType == 'InheritedSalesCatalog')}">
			<c:set var="childTypeLocked" value='readonly="true"'/>
			<%--
				In a sales catalog, the relationship between a sales catalog and a child category belong to the asset store only
				if both the parent sales catalog and child category belong to the asset store.
			--%>
		...
		<object objectType="${childType}" ${childTypeLocked}>
			...
			<c:forEach var="navigationRelationship" items="${catalogGroup.navigationRelationship}">
				<%-- The navigational relationship with no id corresponds to the one to the catalog --%>
				<c:if test="${navigationRelationship.catalogGroupReference.catalogGroupIdentifier.uniqueID == null}">
