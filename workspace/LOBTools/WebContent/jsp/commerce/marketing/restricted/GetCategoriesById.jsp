...
<wcf:getData
	...
	var="categories"
	expressionBuilder="getCatalogGroupWithAllParentDetailsByIDs"
...
<c:forEach var="catalogGroup" items="${categories}">		
	...
	<object objectType="${referenceObjectType}">
		...
		<c:forTokens items="${associatedElementInfo}" delims="," var="categoryInfoFromAssociatedElement">
			<c:forTokens items="${categoryInfoFromAssociatedElement}" delims="|" var="token" begin="0" end="0">
				<c:set var="categoryIdFromAssociatedElement" value="${token}" />
			</c:forTokens>
			<c:if test="${categoryIdFromAssociatedElement == catalogGroup.catalogGroupIdentifier.uniqueID}">
				<c:forTokens items="${categoryInfoFromAssociatedElement}" delims="|" var="element" begin="1" varStatus="status">
					<c:choose>
