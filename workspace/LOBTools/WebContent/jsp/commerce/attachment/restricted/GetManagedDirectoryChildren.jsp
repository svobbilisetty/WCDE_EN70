...
<wcf:getData type="com.ibm.commerce.content.facade.datatypes.ManagedDirectoryType[]"
	...
	<c:choose>
	   <c:when test="${(empty param.parentPath)}">
	      <wcf:param name="parentDirectoryPath" value="/${param.displayName}"/>
	   </c:when>
	   <c:otherwise>
	       <wcf:param name="parentDirectoryPath" value="${param.parentPath}/${param.displayName}"/>
	   </c:otherwise>   
	</c:choose>
</wcf:getData>
