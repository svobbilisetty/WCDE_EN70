...
PhysicalStoreCookieJS = {
	...
	setStoreIdsToCookie:function() {
		...
		if (newCookieValue.length == 0) {
			...
			dojo.cookie("WC_physicalStores", null, {path: "/", expires: -1});
		}
		else {
			/* add or update the cookie */
			dojo.cookie("WC_physicalStores", newCookieValue, {path: "/"});
		}
	},
	...
	setPickUpStoreIdToCookie:function(value) {
		...
		if (newPickUpStoreId != null && newPickUpStoreId != "undefined" && newPickUpStoreId != "") {
			...
			if (newPickUpStoreId != currentPickUpStoreId) {
				dojo.cookie("WC_pickUpStore", null, {path: "/", expires: -1});
				dojo.cookie("WC_pickUpStore", newPickUpStoreId, {path: "/"});
			}
		}
	},
	...
	setValueToCookie:function(cookieKey, cookieValue) {
		...
		if (newValue != null && newValue != "undefined" && newValue != "") {
			...
			if (newValue != currentValue) {
				dojo.cookie(cookieKey, null, {path: "/", expires: -1});
				dojo.cookie(cookieKey, newValue, {path: "/"});
			}
		}
	},
