...
if(typeof(fbIntegrationJS) == "undefined" || !fbIntegrationJS){
	...
	fbIntegrationJS = {
        ...
        //to be used on the dojo.disconnect for Facebook Menu:
	    ...
	    FBinit_Callback : function(appId) {
	        ...
	        this.appId = appId ; 
			if (dojo.isIE <= 7) { // only IE7 and older - to get FB login status, you MUST Include "channelUrl" parameter 
	    ...
	    _displayFacebookMenu : function() { //subscribe to FB events 
		    ...
		    if(logoutFacebookButtonDiv!=null && logoutFacebookButtonDiv != undefined){
				...
				if(showFacebookMenu!=null && showFacebookMenu != undefined){
					...
					if (dojo.isIE >= 8){ // only IE8 and beyond
                ...
                var headerNode = dojo.byId("widget_masthead_links");
                if (headerNode){
                    headerNode.style.zIndex = 999;
                }
				//remove madison's "dijitButtonNode" class, if found, then add our Social class for menu
				if(dojo.hasClass("menu_DropDown_Facebook", "dijitButtonNode")){
					dojo.removeClass("menu_DropDown_Facebook", "dijitButtonNode");
					if (dojo.isIE){
						//make adjustment to the node for IE browser
						if (dojo.isIE >= 8){
			    			dojo.addClass("menu_DropDown_Facebook", "ie8_socialFacebookdijitButtonNodeAdjustment");
			    		}else
			    		   if (dojo.isIE < 8 ){
			    			   dojo.addClass("menu_DropDown_Facebook", "ie_socialFacebookdijitButtonNodeAdjustment");
			    		   }
					}
				}
				...
				var arrowSpanNode = dojo.query('> .socialDropDownArrow', 'menu_DropDown_Facebook')[0];
				arrowSpanNode.innerHTML="";
				var arrowImg=document.createElement("img");
				arrowImg.setAttribute("id", "shopCartArrowImg" );
				arrowImg.setAttribute("src", imageDirectoryPath + "images/colors/color1/widget_minishopcart/minishopcart_arrow.png" );
				arrowSpanNode.appendChild(arrowImg);
