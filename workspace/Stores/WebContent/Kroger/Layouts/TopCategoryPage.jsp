							...
							<div class="main_content">
								...
								<div class="container_content_leftsidebar container_margin_5px">
									...
									<div class="right_column">
											...
											<div class="widget_title_container categorySpot">
												<c:if test="${env_inPreview && !env_storePreviewLink}">
													...
													<div class="ESpotInfo">
														...
														<a id="TopCategoryPage_click2edit_Category_${WCParam.categoryId}" class="click2edit_button"  href="javascript:void(0)" onclick="parent.callManagementCenter('<wcf:out escapeFormat="js" value="${clickToEditURL}"/>');" ><fmt:message bundle='${previewText}' key='Click2Edit_category'/></a>
													</div>
												</c:if>
										...
										<c:set var="eMarketingSpotName2" value="TopBrowsed_CatEntries"/>
										<flow:ifEnabled feature="RemoteWidget"> 
											<c:set var="showFeed2" value="true"/>											
											<c:url var="eMarketingFeedURL2" value="${restURLScheme}://${pageContext.request.serverName}:${restURLPort}${restURI}/stores/${storeId}/MarketingSpotData/${eMarketingSpotName2}">
												<c:param name="responseFormat" value="atom" />
												<c:param name="langId" value="${langId}" />
												<c:param name="currency" value="${env_currencyCode}"/>
											</c:url>
										...
										<div class="widget_product_listing_position">											
												<c:if test="${!env_fetchMarketingDetailsOnLoad}">
													<% out.flush(); %>
													<c:import url="${env_jspStoreDir}Widgets/ESpot/ProductRecommendation/ProductRecommendation.jsp">
														<c:param name="emsName" value="${eMarketingSpotName2}"/>
														<c:param name="pageView" value="miniGrid"/>
														<c:param name="pageSize" value="4"/>
														<c:param name="showFeed" value="${showFeed2}"/>
														<c:param name="feedURL" value="${eMarketingFeedURL2}"/>
														<c:param name="espotTitle" value="${titleString}"/>
														<c:param name="categoryId" value="${WCParam.categoryId}"/>
													</c:import>
													<% out.flush(); %>
