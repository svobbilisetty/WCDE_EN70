...
<form id='<c:out value="${formName}"/>' name='<c:out value="${formName}"/>'>
	...
	<div id="box_1">
			...
			<div id="centered_single_column_form">
				<c:choose>
					<c:when test="${!empty addressId && addressId != -1}">
						<h1><fmt:message key="ADDR_EDIT_ADDRESS"/></h1><br />
					</c:when>
					<c:otherwise>
						<h1><fmt:message key="ADDR_CREATE_ADDRESS"/></h1><br />
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					...
					<c:when test="${locale eq 'ar_EG'}">
						<%@ include file="../../ReusableObjects/ShoppingCartAddressEntryForm_AR.jspf"%>
				...
				<c:if test="${empty addressId || addressId == -1}">
					...
					<span class="checkbox">
						...
						<span class="text"><label for="addToBothShippingBilling"><fmt:message key="AB_SBADDR"/></label></span>
					</span>
				</c:if>
			</div>
			<br clear="all" />
		...
		<div id="centered_single_column_form_footer">
			<a role="button" class="button_primary tlignore" id="WC_ShopcartAddressFormDisplay_links_1" tabindex="0" href="JavaScript:AddressHelper.saveShopCartAddress('<c:out value='${serviceId}'/>', '<c:out value='${formName}'/>');">
