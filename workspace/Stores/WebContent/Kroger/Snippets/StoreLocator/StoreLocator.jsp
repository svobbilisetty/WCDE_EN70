...
<wcf:getData type="com.ibm.commerce.store.facade.datatypes.GeoNodeType[]"
	var="topGeoNodes" varException="topGeoNodeException" expressionBuilder="findStoreTopGeoNodes">
...
<script type="text/javascript">
	...
	<fmt:message key='MISSING_CITY' var="MISSING_CITY"/>
	<fmt:message key='EXCEED_PHYSICAL_STORE_SIZE' var="EXCEED_PHYSICAL_STORE_SIZE"/>
...
<c:choose>
	<c:when test="${fromPage == 'ProductDetails'}">
		...
		<div id="check_product_availability_container" class="contentgrad_header">
			...
			<div id="WC_StoreLocator_div_2" class="left"><span class="contentgrad_text"><fmt:message key="STORELOCATOR_TITLE2" /></span></div>
...
<c:if test="${fromPage == 'ProductDetails'}">
	<c:if test="${!empty productId}">
		<div id="check_product_availability">
			...
			<div id="WC_StoreLocator_div_11" class="font3"><fmt:message key="PRICE_LABEL" /><%@ include file="../ReusableObjects/CatalogEntryPriceDisplay.jspf" %></div>
			</div>
		</div>
    	<br clear="all" />
	</c:if>
...
<c:choose>
	<c:when test="${fromPage != 'ShoppingCart'}">	
		<div class="store_locator_title"><span aria-level="1" role="heading"><fmt:message key="STORELOCATOR_TITLE1"/></span></div>
		<div class="gift_header"><span aria-level="2" role="heading"><fmt:message key="STORE_LIST_TITLE"/></span>
			<div id="hideStoreListHeader" class="checkout_show_icon" style="display:block">
				<div id="WC_StoreLocator_div_14" class="hide"><a role="button" class="tlignore" id="WC_StoreLocator_links_5" href="Javascript:storeLocatorJS.hideStoreList();"><fmt:message key="STORE_LIST_HIDE" /> <img src="<c:out value='${jspStoreImgDir}images/'/>collapse_icon.gif" border="0" alt=""/></a></div>
			</div>
			<div id="showStoreListHeader" class="checkout_show_icon" style="display:none">
				<div id="WC_StoreLocator_div_18" class="hide"><a role="button" class="tlignore" id="WC_StoreLocator_links_6" href="Javascript:storeLocatorJS.showStoreList();"><fmt:message key="STORE_LIST_SHOW" /> <img src="<c:out value='${jspStoreImgDir}images/'/>expand_icon.gif" border="0" alt=""/></a></div>
			</div>
		</div>
	</c:when>
	<c:otherwise>
		<div class="gift_header"><span aria-level="1" role="heading"><fmt:message key="STORE_LIST_TITLE"/></span>
...
<!-- your store list -->
<span id="selectedStoreList_ACCE_Label" class="spanacce"><fmt:message key="ACCE_Region_Your_Store_List"/></span>
<div dojoType="wc.widget.RefreshArea" id="selectedStoreList" widgetId="selectedStoreList" controllerId="selectedStoreListController" style="display:block;" ariaMessage="<fmt:message key="ACCE_Status_Your_Store_List_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="selectedStoreList_ACCE_Label">
	<% out.flush(); %>
...
<c:choose>
	<c:when test="${fromPage != 'ShoppingCart'}">	
		<h2 class="gift_header"><fmt:message key="STORELOCATOR_TITLE1"/></h2>
	</c:when>
	<c:otherwise>
		<h1 class="gift_header"><fmt:message key="STORELOCATOR_TITLE1"/></h1>
	</c:otherwise>
</c:choose>

<div class="gift_content">
	<span class="instruction"><fmt:message key="SELECT_ENTER_OPTIONS"/></span>
	...
	<div id="location">
		...
		<form id='<c:out value="${formName}" />' name='<c:out value="${formName}" />'>
			...
			<div id="WC_StoreLocator_div_29" class="location_select">
				<div id="WC_StoreLocator_div_30" class="location_select_label"><label for="selectCountry"><fmt:message key="SELECT_COUNTRY" /></label></div>
				<div id="WC_StoreLocator_div_31" class="location_select_form">
					<select name="selectCountry" id="selectCountry" title="<fmt:message key='ACCE_COUNTRY_CHANGE'/>" class="drop_down_country" onchange="JavaScript:storeLocatorJS.changeCountrySelection(this.options[this.selectedIndex].value);">
			...
			<div id="WC_StoreLocator_div_32" class="location_select">
				<div id="WC_StoreLocator_div_33" class="location_select_label"><label for="selectState"><fmt:message key="SELECT_STATEPROVINCE" /></label></div>
			...
			<div id="WC_StoreLocator_div_35" class="location_select">
				<div id="WC_StoreLocator_div_36" class="location_select_label"><label for="selectCity"><fmt:message key="SELECT_CITY" /></label></div>
			...
			<div id="WC_StoreLocator_div_38" class="location_select">
				<div id="WC_StoreLocator_div_39" class="location_select_button">
					<a href="#" role="button" class="button_primary" id="cityGo" onclick="Javascript:setCurrentId('cityGo'); storeLocatorJS.refreshResultsFromCity(document.${formName}, '<c:out value='${fromPage}'/>');">
						...
						<div class="button_text"><fmt:message key="GO_BUTTON_LABEL" /></div>
...
<span id="storeLocatorResults_ACCE_Label" class="spanacce"><fmt:message key="ACCE_Region_Store_Result_List"/></span>
<div dojoType="wc.widget.RefreshArea" id="storeLocatorResults" widgetId="storeLocatorResults" ariaMessage="<fmt:message key="ACCE_Status_Store_Result_List_Updated"/>" ariaLiveId="${ariaMessageNode}" controllerId="storeLocatorResultsController" role="region" aria-labelledby="storeLocatorResults_ACCE_Label">
	<% out.flush(); %>
