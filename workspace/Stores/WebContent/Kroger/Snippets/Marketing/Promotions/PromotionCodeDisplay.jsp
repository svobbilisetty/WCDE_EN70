...
<form name="PromotionCodeForm" id="PromotionCodeForm" method="post" action="<c:out value="${PromotionCodeManage}"/>" onsubmit="javascript: return false;">
	...
	<div class="promotion_code" id="WC_PromotionCodeDisplay_div_1">
		<label for="promoCode"><fmt:message key="PROMOTION_CODE"/></label>
	</div>
	...
	<div class="promotion_button" id="WC_PromotionCodeDisplay_div_3">
		<div class="button_align" id="WC_PromotionCodeDisplay_div_4">
				...
				<div class="left_border"></div>
					<div class="button_text"><fmt:message key="APPLY"/><span id="WC_PromotionCodeDisplay_links_1_ACCE_Label" class="spanacce"><fmt:message key="Checkout_ACCE_promo_code_apply" /></span></div>
				...
				<div class="promotion_used">
					...
					<p>
						<a class="font1" id="promotion_${status.count}" href="#" onclick='JavaScript:setCurrentId("promotion_<c:out value='${status.count}'/>");CheckoutHelperJS.removePromotionCode("PromotionCodeForm",<wcf:json object='${aPromotionCode}'/>,"<c:out value='${returnView}'/>");return false;'>
							...
							<fmt:message key="PROMOTION_CODE_REMOVE" /> <c:out value="${promotionCode.code}"/>
						</a>
						<span id="promotionDetailsAcceText_${status.count}" style="display:none">
							<fmt:message key='PROMOTION_DETAILS' bundle='${storeText}'/>
							...
							<c:forEach var="desc" items="${promotionCode.descArray}" varStatus="status2">
								<c:if test = "${!empty desc}">
									...
									<c:out value="${desc}" escapeXml="true"/>
								</c:if>
							</c:forEach>
							<c:if test="${emptyDesc}">
								<fmt:message key="PROMO_NO_DESC" />
							</c:if>								
						</span>
					
						<span class="info_icon" id="promotion_${status.count}_details" tabindex="0" onmouseover="javascript: this.title = '';" onmouseout="javascript: this.title = document.getElementById('promotionDetailsAcceText_<c:out value='${status.count}'/>').innerHTML;"
							title="<fmt:message key='PROMOTION_DETAILS' bundle='${storeText}'/>
								<c:set var="emptyDesc" value="true"/>
								<c:forEach var='desc' items='${promotionCode.descArray}' varStatus='status2'>
									<c:if test = '${!empty desc}'>
										<c:set var='emptyDesc' value='false'/>
								...
								<c:if test='${emptyDesc}'>
									<fmt:message key='PROMO_NO_DESC' bundle='${storeText}'/>
								</c:if>
							">
							<img src="<c:out value='${jspStoreImgDir}${vfileColor}/info.png'/>"  alt="<fmt:message key="DETAILS" />"/>
							...
							<div class="content_right_border">
								<div class="content">
									<div class="header" id="WC_PromotionCodeDisplay_div_6_${status.count}"> 
										<span id="WC_PromotionCodeDisplay_div_7_${status.count}"><fmt:message key="PROMOTION_DETAILS" /></span>
										<div class="clear_float"></div>
									</div>
									...
									<div class="body" style="height:${descStringLen/2}px;overflow:hidden;" id="WC_PromotionCodeDisplay_div_9_${status.count}">
										...
										<c:if test="${emptyDesc}">
											<div id="WC_PromotionCodeDisplay_div_12_${status.count}">
												...
												<fmt:message key="PROMO_NO_DESC" /><br />
											</div>
										</c:if>
									</div>
								</div>
