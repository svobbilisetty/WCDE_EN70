...
<div class="card_info" id="WC_StandardAmex_div_1_<c:out value='${paymentAreaNumber}'/>">
	<c:choose>
	...
	<input type="hidden" name="cc_brand" value="<c:out value="${cc_brand}" />" id="WC_StandardAmex_inputs_1_<c:out value='${paymentAreaNumber}'/>"/>
	<input type="hidden" name="paymentTCId" value="<c:out value="${paymentTCId}"/>" id="WC_StandardAmex_inputs_2_<c:out value='${paymentAreaNumber}'/>"/>
	...
	<span class="col1">
		<span class="card_number">
			<span>
				<span class="required-field">*</span><label for="edp_Amex_account_<c:out value='${paymentAreaNumber}' />">
				<span><fmt:message key="EDPPaymentMethods_AMEX_CARD_NUMBER" /></span></label>
			</span>
			<span>
				<input aria-required="true" onchange="JavaScript:TealeafWCJS.processDOMEvent(event);CheckoutPayments.removeCreditCardNumberAndCVV(<c:out value='${paymentAreaNumber}'/>, false, true);CheckoutHelperJS.paymentDataDirty(<c:out value='${paymentAreaNumber}'/>, true);CheckoutPayments.updatePaymentObject(<c:out value='${paymentAreaNumber}'/>, 'account');"" type="tel" name="account1" style="width:142px" value="<c:out value="${accountValue}"/>" id="account1_<c:out value='${paymentAreaNumber}'/>"/>
			</span>
		</span>	
		<span>
		...
		<span class="visaCVV2">
			<span id="CVVDetailsSection_<c:out value='${paymentAreaNumber}'/>">		
				<span id="StandardAmex_CVVDetails_AcceText_<c:out value='${paymentAreaNumber}'/>" style="display:none">
					<fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" />
					<fmt:message key="EDPPaymentMethods_CVV_DEFINITION" />
				</span>
				<span id="StandardAmex_CVVDetails_<c:out value='${paymentAreaNumber}'/>" class="order_link" title="<fmt:message key='EDPPaymentMethods_CVV_WHAT_IS' bundle='${storeText}'/><fmt:message key='EDPPaymentMethods_CVV_DEFINITION' bundle='${storeText}'/>" onmouseover="javascript: this.title = '';" onmouseout="javascript: this.title = document.getElementById('StandardAmex_CVVDetails_AcceText_<c:out value='${paymentAreaNumber}'/>').innerHTML;" tabindex="0"><fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" /></span>
				<span id="StandardAmex_CVVDetails_<c:out value='${paymentAreaNumber}'/>_span_2" dojoType="wc.widget.Tooltip" connectId="StandardAmex_CVVDetails_<c:out value='${paymentAreaNumber}'/>" style="display: none;">
						...
						<div class="middle">
							<div class="content_left_border">
								<div class="content_right_border">
									<div class="content">
										<div id="WC_StandardAmex_div_2_<c:out value='${paymentAreaNumber}'/>" class="header">
											<span><fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" /></span>
										</div>
						
										<div id="WC_StandardAmex_div_4_<c:out value='${paymentAreaNumber}'/>" class="body">
											<fmt:message key="EDPPaymentMethods_CVV_DEFINITION" />
										</div>
									</div>
								</div>
				...
				<script type="text/javascript">dojo.addOnLoad(function() {parseWidget("StandardAmex_CVVDetails_<c:out value='${paymentAreaNumber}'/>_span_2"); });</script>
			</span>
		</span>
		<span class="date">
