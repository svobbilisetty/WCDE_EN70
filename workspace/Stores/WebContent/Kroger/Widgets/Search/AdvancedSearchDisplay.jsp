...
<wcf:getData type="com.ibm.commerce.infrastructure.facade.datatypes.OnlineStoreType"
		...
		<wcf:param name="pageName" value="ADVANCED_SEARCH_PAGE"/>
...
xmlns:waistate="http://www.w3.org/2005/07/aaa" lang="${shortLocale}" xml:lang="${shortLocale}">
	<head>
		...
		<title><fmt:message key="TITLE_ADVANCED_SEARCH"/></title>
									...
									<div id="box">
										<div class="sign_in_registration" id="WC_AdvancedSearchForm_div_1">
											<div class="title" id="WC_AdvancedSearchForm_div_2">
												<h1>
													<fmt:message key="TITLE_ADVANCED_SEARCH"/>
												</h1>
											</div>
											<form name="AdvancedSearchForm" action="SearchDisplay" method="get" id="AdvancedSearchForm">
												...
												<div class="form" id="WC_AdvancedSearchForm_div_3">
													<div class="myaccount_header" id="WC_AdvancedSearchForm_div_4">
														<h2 class="registration_header"><fmt:message key="ADVANCED_SEARCH_MESSAGE"/></h2>
													</div>
														...
														<div class="align" id="WC_AdvancedSearchForm_div_6">
															<div class="form_2column" id="WC_AdvancedSearchForm_div_7">
																<div class="column" id="WC_AdvancedSearchForm_div_8">
																	...
																	<label for="WC_AdvancedSearchForm_FormInput_searchType" class="nodisplay"><fmt:message key="SEARCH_FOR_OPTIONS" /></label>
						                                             ...
						                                             <div class="column_label" id="WC_AdvancedSearchForm_div_11">
																		<span class="spanacce">
																			<label for="WC_AdvancedSearchForm_FormInput_filterTerm">
																				<fmt:message key="EXCLUDE_WORDS"/>
																			</label>
																		</span>
																		<fmt:message key="EXCLUDE_WORDS"/>
																	</div>
																	<input size="46" maxlength="50" aria-required="true" name="filterTerm" id="WC_AdvancedSearchForm_FormInput_filterTerm" value="<c:out value="${WCParam.filterTerm}"/>" />
																	...
																	<div class="column_label" id="WC_AdvancedSearchForm_div_13">
																		<span class="spanacce">
																			<label for="WC_AdvancedSearchForm_FormInput_categoryId">
																				<fmt:message key="SEARCH_IN1"/>
																			</label>
																		</span>
																		<fmt:message key="SEARCH_IN1"/>
																	</div>
																	<select name="categoryId" id="WC_AdvancedSearchForm_FormInput_categoryId">
																		<option value=""><fmt:message key="SEARCH_ALL_DEPARTMENTS" /></option>
																	...
																	<div class="column_label" id="WC_AdvancedSearchForm_div_20">
																		<span class="spanacce">
																			<label for="WC_AdvancedSearchForm_FormInput_manufacturer">
																				<fmt:message key="SEARCH_BRANDS"/>
																			</label>
																		</span>
																		<fmt:message key="SEARCH_BRANDS"/>
						                                             </div>
						                                             <input type="text" size="46" maxlength="50" aria-required="true" name="manufacturer" id="WC_AdvancedSearchForm_FormInput_manufacturer" value="<c:out value="${WCParam.manufacturer}"/>" />
						                                        </div>
																<div class="column" id="WC_AdvancedSearchForm_div_21">
																	<div class="column_label" id="WC_AdvancedSearchForm_div_22">
																		<span class="spanacce">
																			<label for="WC_AdvancedSearchForm_FormInput_searchTermScope">
																				<fmt:message key="SEARCH_FOR_WORD_IN"/>
																			</label>
																		</span>
																		<fmt:message key="SEARCH_FOR_WORD_IN"/>
																	</div>
																	<select size="1" name="searchTermScope" id="WC_AdvancedSearchForm_FormInput_searchTermScope">
																		...
																		<option value="3" <c:if test="${!empty WCParam.searchTermScope && WCParam.searchTermScope=='3'}">selected="selected"</c:if>>
																			<fmt:message key="PRODUCT_ATTACHMENT_ONLY" />
																		</option>
																	</select>
																</div>
																...
																<c:if test="${globalpricemode == null}">
																	<wcf:getData type="com.ibm.commerce.catalog.facade.datatypes.CatalogNavigationViewType" var="catalogNavigationView" 
																		expressionBuilder="getCatalogNavigationView" varShowVerb="showCatalogNavigationView" recordSetStartNumber="0" maxItems="1">
																		<wcf:contextData name="storeId" data="${storeId}" />
																		<wcf:contextData name="catalogId" data="${catalogId}" />
																		<wcf:param name="searchProfile" value="IBM_findCatalogEntryAll_PriceMode"/>
																	</wcf:getData>
																	...
																	<c:when test="${globalpricemode != 0}">
																		<div class="column" id="WC_AdvancedSearchForm_div_23">
																			<div class="column_label" id="WC_AdvancedSearchForm_div_24">
																				<span class="spanacce">
																					<label>
																						<fmt:message key="SEARCH_PRICE_RANGE"/>
																					</label>
																				</span>
																				<fmt:message key="SEARCH_PRICE_RANGE"/>
																			</div>
																			<div class="priceRangeContainer">
																				<div class="range">
																					<span class="label"><c:out value="${env_CurrencySymbolToFormat}" escapeXml="false"/></span><span class="inputbox"><input type="text" size="18" maxlength="50" aria-required="true" title='<fmt:message key="SEARCH_PRICE_RANGE"/> <fmt:message key="LN_SEARCH_FACET_LOWER_BOUND"/>' name="minPrice" id="WC_AdvancedSearchForm_FormInput_minPrice" value="<c:out value="${WCParam.minPrice}"/>"/></span>
																				</div>
																				<div>-</div>
																				<div class="range">
																					<span class="label"><c:out value="${env_CurrencySymbolToFormat}" escapeXml="false"/></span><span class="inputbox"><input type="text" size="19" maxlength="50" aria-required="true" title='<fmt:message key="SEARCH_PRICE_RANGE"/> <fmt:message key="LN_SEARCH_FACET_UPPER_BOUND"/>' name="maxPrice" id="WC_AdvancedSearchForm_FormInput_maxPrice" value="<c:out value="${WCParam.maxPrice}"/>"/></span>
																				</div>
																			</div>
																		</div>
																	...
																	<div class="column_label" id="WC_AdvancedSearchForm_div_26">
																		<span class="spanacce">
																			<label for="WC_AdvancedSearchForm_FormInput_pageSize">
																				<fmt:message key="RESULTS_PER_PAGE"/>
																			</label>
																		</span>
																		<fmt:message key="RESULTS_PER_PAGE"/>
																	</div>
													...
													<div class="button_footer_line no_float" id="WC_AdvancedSearchForm_div_40">
														<a href="#" role="button" class="button_primary" id="WC_AdvancedSearchForm_links_1" tabindex="0" onclick="SearchJS.validateForm($('AdvancedSearchForm'));">
															<div class="left_border"></div>
															<div class="button_text"><fmt:message key="SEARCH_CATALOG"/></div>												
	...
	<script type="text/javascript">
		dojo.addOnLoad(function() {
			...
			<fmt:message key="ERROR_PRICE_RANGE" var="ERROR_PRICE_RANGE" />
