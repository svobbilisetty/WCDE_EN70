...
<div class="main_header" id="WC_ShipmentDisplay_div_5">
	...
	<div class="left" id="WC_ShipmentDisplay_div_7"><span aria-level="1" class="main_header_text" role="heading"><fmt:message key="BCT_SHIPPING_INFO"/></span></div>
	...
	<c:when test = "${shipmentTypeId == 1}">
		<flow:ifEnabled  feature="MultipleShipments">	
		   <div class="checkout_subheader" id="WC_ShipmentDisplay_div_9">				
				<div class="left" id="WC_ShipmentDisplay_div_11"><span class="content_text"><fmt:message key="SHIP_MULTIPLE_SHIPMENT_MESSAGE"/></span></div>
				<a role="button" class="button_primary header_element_align" id="WC_ShipmentDisplay_links_3" tabindex="0" href="javascript:setPageLocation('<c:out value="${MultipleShipmentPageURL}"/>')">
					...
					<div class="button_text"><fmt:message key="SHIP_MULTIPLE_SHIPMENTS" /><span class="spanacce"><fmt:message key="Checkout_ACCE_Multi_Shipping"/></span></div>
	...
	<c:otherwise>
		<c:if test="${!hideSingleShipment}">
		 <div class="checkout_subheader" id="WC_MultipleShipmentDisplay_div_9">
			...
			<c:forEach var="shipMode" items="${orderShipModeInfo.orderItem[0].usableShippingMode}">
				<c:set var="shipModeId" value="${shipMode.shippingModeIdentifier.uniqueID}"/>
					<c:if test="${((!b2bStore && (shipModeMap[shipModeId] == orderItemCount)) || shipModeMap[shipModeId] == numberOfOrderItemsToInspect) && empty commonShipModeId && shipMode.shippingModeIdentifier.externalIdentifier.shipModeCode != 'PickupInStore'}">
			...
			<div class="left" id="WC_MultipleShipmentDisplay_div_11"><span class="content_text"><fmt:message key="SHIP_SINGLE_SHIPMENT_MESSAGE"/></span></div>
			<div class="header_element_align" id="WC_MultipleShipmentDisplay_div_12">
				<a role="button" class="button_primary" id="WC_MultipleShipmentDisplay_links_3" tabindex="0" href="JavaScript:setCurrentId('WC_MultipleShipmentDisplay_links_3'); CheckoutHelperJS.moveAllItemsToSingleShipment()">
					<div class="left_border"></div>
					<div class="button_text"><fmt:message key="SHIP_SINGLE_SHIPMENT"/><span class="spanacce"><fmt:message key="Checkout_ACCE_Single_Shipping" /></span></div>
					<div class="right_border"></div>
				</a>
			</div>
			<div id="WC_ShipmentDisplay_div_14a" class="header_element_align">
	...
	<div id="shipping">
		<c:choose>
			<c:when test = "${shipmentTypeId == 1}">
				...
				<div dojoType="wc.widget.RefreshArea" widgetId="singleShipmentOrderDetails" controllerId="traditionalShipmentDetailsController" ariaMessage="<fmt:message key="ACCE_Status_Order_Item_List_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="singleShipmentOrderDetails_ACCE_Label" id="WC_ShipmentDisplay_div_17">
					<%out.flush();%>
			...
			<c:otherwise>
				...
				<div id="WC_MultipleShipmentDisplay_div_17">
					<flow:ifEnabled feature="ShipAsComplete">
						<br/>
						<span class="checkbox">
								<input type="checkbox" title="<fmt:message key="SHIP_SHIP_AS_COMPLETE" />" class="checkbox" id="shipAsComplete" name = "shipAsComplete" onClick="setCurrentId('shipAsComplete'); CheckoutHelperJS.shipAsComplete(this)"
								<c:if test="${shipAsCompleteCheckBoxStatus}">
									checked="checked"
								</c:if> />
							<span class="text"><label for="shipAsComplete"><fmt:message key="SHIP_SHIP_AS_COMPLETE" /></label></span>
						</span>
					</flow:ifEnabled>
				</div>
				...
				<span id="multipleShipmentOrderDetails_ACCE_Label" class="spanacce"><fmt:message key="ACCE_Region_Order_Item_List"/></span>
				<div id="WC_MultipleShipmentDisplay_div_18" class="shipping_billing_img_padding" dojoType="wc.widget.RefreshArea" widgetId="multipleShipmentOrderDetails" controllerId="multipleShipmentDetailsController" ariaMessage="<fmt:message key="ACCE_Status_Order_Item_List_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="multipleShipmentOrderDetails_ACCE_Label">
					<%out.flush();%>
