...
<table id="order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message key="SHOPCART_TABLE_SUMMARY"  />">
	<tr class="nested">
		...
		<th class="align_left" id="MultipleShipments_tableCell_availability"><fmt:message key="AVAILABILITY"  /></th>
		<th class="align_center" id="MultipleShipments_tableCell_quantity" abbr="<fmt:message key="QUANTITY1"  />"><fmt:message key="QTY"  /></th>
		<th class="align_right" id="MultipleShipments_tableCell_unitPrice" abbr="<fmt:message key="UNIT_PRICE"  />"><fmt:message key="EACH"  /></th>
		<th class="align_right" id="MultipleShipments_tableCell_totalPrie" abbr="<fmt:message key="TOTAL_PRICE"  />"><fmt:message key="TOTAL"  /></th>
	</tr>
	...
	<c:forEach var="orderItem" items="${pgorder.orderItem}" varStatus="status">
		...
		<tr>
			 <th class="th_align_left_normal <c:out value="${nobottom}"/>" id="MultipleShipping_rowHeader_product<c:out value='${status.count}'/>" abbr="<fmt:message key="Checkout_ACCE_for"  /> ${catEntry.name}" width="225">
				<div class="img" id="WC_MSOrderItemDetails_div_1_<c:out value='${status.count}'/>">
					...
					<a href="${catEntryDisplayUrl}" id="catalogEntry_img_${orderItem.orderItemIdentifier.uniqueID}" title="${catEntry.name}">
				...
				<div class="itemspecs hover_underline" id="WC_MSOrderItemDetails_div_2_<c:out value='${status.count}'/>">
					<p><a class="hover_underline" id="catalogEntry_name_${orderItem.orderItemIdentifier.uniqueID}" href="<c:out value="${catEntryDisplayUrl}"/>"><c:out value="${catEntry.name}" escapeXml="false"/></a></p>
					
					<span><fmt:message key="CurrentOrder_SKU"  /> <c:out value="${catEntry.partNumber}" escapeXml="false"/></span>
					<br />
							...
							<c:forEach var="attribute" items="${catalogEntry1.catalogEntryAttributes.attributes}" varStatus="status3">
								...
								<c:if test="${!empty selectedAttr && childrenSKUCount > 1}">
									<a class="order_link tlignore" id="WC_MSOrderItemDetails_link_1_<c:out value='${status.count}'/>_<c:out value='${status3.count}'/>" href="javaScript:if(!this.disabled) {setCurrentId('WC_MSOrderItemDetails_link_1_<c:out value='${status.count}'/>_<c:out value='${status3.count}'/>');
										QuickInfoJS.changeAttributes('${itemUniqueId}','${parentCatEntryId}', '${orderItem.catalogEntryIdentifier.uniqueID}',${quickCartOrderItemQuantity});}"> 
			...
			<c:forEach var="discountsIterator" items="${discountReferences}" varStatus="status2">									
				...
				<c:if test="${discounts.displayLevel == 'OrderItem'}">
					<tr>
						...
						<td colspan="5" class="th_align_left_dotted_top_solid_bottom" abbr="<fmt:message key="Checkout_ACCE_prod_discount"  /> ${catEntry.name}" id="MultipleShipment_rowHeader_discount<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>">
