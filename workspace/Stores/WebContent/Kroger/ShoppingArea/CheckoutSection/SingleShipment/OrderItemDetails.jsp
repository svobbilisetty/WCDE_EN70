...
<input type="hidden" name="currentPageNumber" value="${currentPage}" id="currentPageNumber"/>
 ...
 <table id="order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message key="SHOPCART_TABLE_CONFIRM_SUMMARY"  />">
	...
	<tr class="orderDetailsHeader"><td colspan="5"><fmt:message key="ORDER_SUMMARY"  /></td></tr>
	</c:if>
	  <tr class="nested">
		   <th class="align_left" id="SingleShipment_tableCell_productName"><fmt:message key="PRODUCT"  /></th>
		   <flow:ifEnabled feature="ExpeditedOrders"><th class="align_center" id="SingleShipment_tableCell_expedite"><fmt:message key="SHIP_EXPEDITE_SHIPPING"  /></th></flow:ifEnabled>
	...
	<c:forEach var="orderItem" items="${pgorder.orderItem}" varStatus="status">
				...
				<td class="expedite <c:out value="${nobottom}"/>" id="WC_OrderItemDetails_td_expedite_<c:out value='${status.count}'/>" headers="SingleShipment_tableCell_expedite SingleShipment_rowHeader_product<c:out value='${status.count}'/>">
					<span class="checkbox">
						...
						<label for="expediteShipping_<c:out value='${status.count}'/>">
							<span style="display:none"><fmt:message key="SHIP_EXPEDITE_SHIPPING"  /></span>
						</label>
					</span>
				</td>
			</flow:ifEnabled>
		...
		<c:if test="${!empty orderItem.orderItemAmount.adjustment}">
			...
			<c:forEach var="discountsIterator" items="${discountReferences}" varStatus="status2">									
				...
				<c:if test="${discounts.displayLevel == 'OrderItem'}">
					<tr>
						<th colspan="4" class="th_align_left_dotted_top_solid_bottom" abbr="<fmt:message key="Checkout_ACCE_prod_discount"  /> ${catEntry.name}" id="SingleShipment_rowHeader_discount<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>">
