...
<c:if test="${empty paymentMethodSelected}">
	<c:forEach var="paymentInfo" items="${usablePayments.usablePaymentInformation}">
		<c:if test="${empty paymentMethodSelected}">
			<c:if test="${!empty paymentInfo.paymentMethod.paymentMethodName}">
				<c:set var="paymentMethodSelected" value="${paymentInfo.paymentMethod.paymentMethodName}" />
			</c:if>
		</c:if>
	</c:forEach>
</c:if>
...
<div class="billing_address" id="WC_BillingAddressDropDownDisplay_div_<c:out value='${param.paymentAreaNumber}'/>">
	...
	<fmt:message key="BILL_BILLING_ADDRESS_COLON" />
	<c:if test="${hasValidAddresses}"></label></c:if>
						...
						<c:choose>
							<c:when test="${payment.paymentMethod.paymentMethodName eq paymentMethodSelected}">
								<c:forEach var="addressInPayment" items="${payment.usableBillingAddress}">
									...
									<option <c:out value="${selectStr}" escapeXml='false'/> value="${addressInPayment.uniqueID}"><c:choose><c:when test="${addressInPayment.externalIdentifier.contactInfoNickName eq  profileShippingNickname}"><fmt:message key="QC_DEFAULT_SHIPPING" /></c:when>
									<c:when test="${addressInPayment.externalIdentifier.contactInfoNickName eq  profileBillingNickname}"><fmt:message key="QC_DEFAULT_BILLING" /></c:when><c:otherwise>${addressInPayment.externalIdentifier.contactInfoNickName}</c:otherwise></c:choose></option>
		...
		<c:if test="${selectedAddressId != -1 && hasValidAddresses}" >		
			...
			<div id="editBillingAddressLink_<c:out value='${param.paymentAreaNumber}'/>" class="editAddressLink hover_underline">
					<a class="tlignore" href="JavaScript:CheckoutPayments.saveBillingAddressDropDownBoxContextProperties('edit','<c:out value="${param.paymentAreaNumber}"/>');JavaScript:CheckoutHelperJS.editBillingAddress('0','<c:out value="${param.paymentAreaNumber}"/>','<c:out value='${fn:replace(profileshipping,search01,replaceStr01)}'/>','<c:out value='${fn:replace(profilebilling,search01,replaceStr01)}'/>');CheckoutHelperJS.setLastAddressLinkIdToFocus('editBillingAddressLink_a_edit_<c:out value='${param.paymentAreaNumber}'/>');" id="editBillingAddressLink_a_edit_<c:out value='${param.paymentAreaNumber}'/>">
