...
<%@ include file="../../include/ErrorMessageSetup.jspf" %>
		...
		<flow:ifEnabled feature="Analytics">
			<flow:ifDisabled feature="AjaxCheckout">
				...
				<cm:cart orderType="${orderForAn}" extraparms="null, ${analyticsFacet}"/>
			</flow:ifDisabled>
...
<html xmlns:wairole="http://www.w3.org/2005/01/wai-rdf/GUIRoleTaxonomy#" xmlns:waistate="http://www.w3.org/2005/07/aaa" lang="${shortLocale}" xml:lang="${shortLocale}">
	<head>
		...
		<title><fmt:message key="SHOPPINGCART_TITLE"/></title>
		...
		<script type="text/javascript">
			dojo.addOnLoad(function() { 
				...
				<fmt:message key="REQUIRED_SPECIFIC_FIELD_ENTER" var="LOGON_REQUIRED_FIELD_ENTER">
					<fmt:param><fmt:message key="SHOPCART_USERNAME"/></fmt:param>
				</fmt:message>
				<fmt:message key="REQUIRED_SPECIFIC_FIELD_ENTER" var="PASSWORD_REQUIRED_FIELD_ENTER">
					<fmt:param><fmt:message key="SHOPCART_PASSWORD"/></fmt:param>
				</fmt:message>
				...
				<fmt:message key="ERROR_UPDATE_FIRST_SHOPPING_CART" var="ERROR_UPDATE_FIRST_SHOPPING_CART"/>
				...
				<fmt:message key="BOPIS_QUICKCHECKOUT_ERROR" var="quickCheckoutMsg"/>
				<fmt:message key="ERR_NO_PHY_STORE" var="ERR_NO_PHY_STORE"/>
				<fmt:message key="GENERICERR_MAINTEXT" var="ERROR_RETRIEVE_PRICE">                                     
				<fmt:param><fmt:message key="GENERICERR_CONTACT_US"/></fmt:param></fmt:message>
				<fmt:message key="INVENTORY_ERROR" var="ERROR_RETRIEVE_PRICE_QTY_UPDATE"/>
				<fmt:message key="SHOPCART_HAS_NON_RECURRING_PRODUCTS" var="RECURRINGORDER_ERROR"/>
				...
				<c:if test="${b2bStore}">	
					savedOrdersJS.setCommonParameters('<c:out value="${WCParam.langId}"/>','<c:out value="${WCParam.storeId}"/>','<c:out value="${WCParam.catalogId}"/>');
					<%-- ADD_BUTTON message key missing --%>
					savedOrdersJS.setImgParameters("ADD_ENABLED","<c:out value='${jspStoreImgDir}' />images/add_reqlist.gif","<fmt:message key="ADD_BUTTON"/>");
					<%-- ADD_BUTTON_DISABLED message key missing --%>
					savedOrdersJS.setImgParameters("ADD_DISABLED","<c:out value='${jspStoreImgDir}' />images/add_reqlist_disabled.gif","<fmt:message key="ADD_BUTTON_DISABLED"/>");
							...
							<div class="main_content">
								...
								<div class="container_content_rightsidebar">
									<div class="left_column"> 
										...
										<span id="ShopCartDisplay_ACCE_Label" class="spanacce"><fmt:message key="ACCE_Region_Shopping_Cart_Content"/></span>
										<div dojoType="wc.widget.RefreshArea" widgetId="ShopCartDisplay" id="ShopCartDisplay" controllerId="ShopCartDisplayController" ariaMessage="<fmt:message key="ACCE_Status_Shopping_Cart_Content_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="ShopCartDisplay_ACCE_Label">
											<%out.flush();%>
