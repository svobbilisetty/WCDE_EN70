...
<jsp:useBean id="policiesForResourceBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.PolicyLightListBeanUtility" >
<jsp:setProperty property="languageId" name="policiesForResourceBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      policiesForResourceBean.setSortAtt( sort );
     
      String resourceId = (String) request.getParameter("resourceId");
   policiesForResourceBean.setResourceId(Integer.valueOf(resourceId)) ;
 com.ibm.commerce.beans.DataBeanManager.activate(policiesForResourceBean, request);
   PolicyLightDataBean[] aList = policiesForResourceBean.getPolicyBeans();
%>
...
function getResultsSize() { 
     return <%= policiesForResourceBean.getPolicyBeans().length %>; 
}
...
{
  for (var i=0; i < document.PoliciesForResourceViewForm.elements.length; i++) 
  {
     var e = document.PoliciesForResourceViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = policiesForResourceBean.getPolicyBeans().length;
...
<%= comm.addControlPanel("policyeditor.PoliciesForResourceView",totalpage,totalsize,locale) %>
<FORM NAME="PoliciesForResourceViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyPoliciesForResourceView"))); 
   }
%>
