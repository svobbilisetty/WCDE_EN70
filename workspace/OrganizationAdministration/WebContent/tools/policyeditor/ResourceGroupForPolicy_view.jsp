...
<jsp:useBean id="resourceGroupForPolicyBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ResourceGroupLightListBeanUtility" >
<jsp:setProperty property="languageId" name="resourceGroupForPolicyBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      resourceGroupForPolicyBean.setSortAtt( sort );
     
  String policyId = (String) request.getParameter("policyId");
   resourceGroupForPolicyBean.setPolicyId(Integer.valueOf(policyId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(resourceGroupForPolicyBean, request);
   ResourceGroupLightDataBean[] aList = resourceGroupForPolicyBean.getResourceGroupBeans();
%>
...
function getResultsSize() { 
     return <%= resourceGroupForPolicyBean.getResourceGroupBeans().length %>; 
}
...
{
  for (var i=0; i < document.ResourceGroupForPolicyViewForm.elements.length; i++) 
  {
     var e = document.ResourceGroupForPolicyViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = resourceGroupForPolicyBean.getResourceGroupBeans().length;
...
<%= comm.addControlPanel("policyeditor.ResourceGroupForPolicyView",totalpage,totalsize,locale) %>
<FORM NAME="ResourceGroupForPolicyViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyResourceGroupForPolicyView"))); 
   }
%>
