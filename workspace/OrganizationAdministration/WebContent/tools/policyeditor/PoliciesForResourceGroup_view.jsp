...
<jsp:useBean id="policiesForResourceGroupBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.PolicyLightListBeanUtility" >
<jsp:setProperty property="languageId" name="policiesForResourceGroupBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
     ...
     if ( sortby != null && !sortby.equals("null") && !sortby.equals("") ) 
      ...
      policiesForResourceGroupBean.setSortAtt( sort );
     
      String resourceGroupId= (String) request.getParameter("resourceGroupId");
   policiesForResourceGroupBean.setResourceGroupId(Integer.valueOf(resourceGroupId)) ;
 com.ibm.commerce.beans.DataBeanManager.activate(policiesForResourceGroupBean, request);
   PolicyLightDataBean[] aList = policiesForResourceGroupBean.getPolicyBeans();
%>
...
function getResultsSize() { 
     return <%= policiesForResourceGroupBean.getPolicyBeans().length %>; 
}
...
{
  for (var i=0; i < document.PoliciesForResourceGroupViewForm.elements.length; i++) 
  {
     var e = document.PoliciesForResourceGroupViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = policiesForResourceGroupBean.getPolicyBeans().length;
...
<%= comm.addControlPanel("policyeditor.PoliciesForResourceGroupView",totalpage,totalsize,locale) %>
<FORM NAME="PoliciesForResourceGroupViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyPoliciesForResourceGroupView"))); 
   }
%>
