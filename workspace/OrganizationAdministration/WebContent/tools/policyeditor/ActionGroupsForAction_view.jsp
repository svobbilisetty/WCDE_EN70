...
<jsp:useBean id="actionGroupForActionBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ActionGroupLightListBeanUtility" >
<jsp:setProperty property="languageId" name="actionGroupForActionBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      actionGroupForActionBean.setSortAtt( sort );
     
  String actionId = (String) request.getParameter("actionId");
   actionGroupForActionBean.setActionId(Integer.valueOf(actionId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(actionGroupForActionBean, request);
   ActionGroupLightDataBean[] aList = actionGroupForActionBean.getActionGroupBeans();
%>
...
function getResultsSize() { 
     return <%= actionGroupForActionBean.getActionGroupBeans().length %>; 
}
...
{
  for (var i=0; i < document.ActionGroupsForActionViewForm.elements.length; i++) 
  {
     var e = document.ActionGroupsForActionViewForm.elements[i];
...
top.setContent(getShowActionsBCT(),
       top.getWebappPath() +'NewDynamicListView?ActionXMLFile=policyeditor.ActionsForActionGroupView&amp;cmd=ActionsForActionGroupView&amp;actionGroupId=' + checked + '&amp;ownerId=' + getOwnerId(), true)
}
...
<%
          ...
          int rowselect = 1;
          int totalsize = actionGroupForActionBean.getActionGroupBeans().length;
...
<%= comm.addControlPanel("policyeditor.ActionGroupsForActionView",totalpage,totalsize,locale) %>
<FORM NAME="ActionGroupsForActionViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyActionGroupsForActionView"))); 
   }
%>
