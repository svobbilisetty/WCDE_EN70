...
<jsp:useBean id="resourcesForPolicyBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ResourceLightListBeanUtility" >
<jsp:setProperty property="languageId" name="resourcesForPolicyBean" value="<%= lang %>" />
</jsp:useBean>
...
PolicySortingAttribute sort = new PolicySortingAttribute();
      ...
      resourcesForPolicyBean.setSortAtt( sort );
     
  String policy_id = (String) request.getParameter("policyId");
   resourcesForPolicyBean.setPolicyId(Integer.valueOf(policy_id)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(resourcesForPolicyBean, request);
   ResourceLightDataBean[] aList = resourcesForPolicyBean.getResourceBeans();
%>
...
function getResultsSize() { 
     return <%= resourcesForPolicyBean.getResourceBeans().length %>; 
}
...
{
  for (var i=0; i < document.ResourcesForPolicyViewForm.elements.length; i++) 
  {
     var e = document.ResourcesForPolicyViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = resourcesForPolicyBean.getResourceBeans().length;
...
<%= comm.addControlPanel("policyeditor.ResourcesForPolicyView",totalpage,totalsize,locale) %>
<FORM NAME="ResourcesForPolicyViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyResourcesForPolicyView"))); 
   }
%>
