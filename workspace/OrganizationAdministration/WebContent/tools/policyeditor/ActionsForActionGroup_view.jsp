...
<jsp:useBean id="actionsForActionGroupBean" class="com.ibm.commerce.accesscontrol.policyeditor.beans.ActionLightListBeanUtility" >
<jsp:setProperty property="languageId" name="actionsForActionGroupBean" value="<%= lang %>" />
</jsp:useBean>
...
<%
      ...
      actionsForActionGroupBean.setSortAtt( sort );
     
  String actionGroupId = (String) request.getParameter("actionGroupId");
   actionsForActionGroupBean.setActionGroupId(Integer.valueOf(actionGroupId)) ;

   com.ibm.commerce.beans.DataBeanManager.activate(actionsForActionGroupBean, request);
   ActionLightDataBean[] aList = actionsForActionGroupBean.getActionBeans();
%>
...
function getResultsSize() { 
     return <%= actionsForActionGroupBean.getActionBeans().length %>; 
}
...
{
  for (var i=0; i < document.ActionsForActionGroupViewForm.elements.length; i++) 
  {
     var e = document.ActionsForActionGroupViewForm.elements[i];
...
<%
          ...
          int rowselect = 1;
          int totalsize = actionsForActionGroupBean.getActionBeans().length;
...
<%= comm.addControlPanel("policyeditor.ActionsForActionGroupView",totalpage,totalsize,locale) %>
<FORM NAME="ActionsForActionGroupViewForm" action="" method="GET">
...
<P>
<% 
     out.println(UIUtil.toHTML((String)PolicyUtilityNLS.get("emptyActionsForActionGroupView"))); 
   }
%>
