...
function getResultsSize() { 
    ...
    function newMemberGroup()
    {
      ...
      url += "&memberGroupType=<%= memberGroupType %>";
      if (top.setContent)
      {
    ...
    function changeMemberGroup()
    {
      ...
      if (changed != 0)
      {
        ...
        <% } %>        
        url += "&segmentId=" + memberGroupId + "&memberGroupType=<%= memberGroupType %>";
        if (top.setContent)
        {
      ...
      if (checked.length > 0)
      {
             ...
             confirmDialog("<%=UIUtil.toJavaScript((String)memberGroupListNLS.get("memberGroupListDeleteConfirmation")) %>") ) 
        {
          ...
          var url = top.getWebappPath() + "SegmentDelete?"
                    ...
                    + "&size=" + checked.length
	            + "&URL=" + redirectURL
	            + "&amp;ActionXMLFile=<%= returnXML %>&amp;cmd=BuyMemberGroupListView&amp;memberGroupType=<%= memberGroupType %>" ;
          if (top.setContent)
          {
            top.showContent(url); 
            top.refreshBCT(); 
...
%>
        alertDialog('<%=UIUtil.toHTML((String)memberGroupListNLS.get("memberGroupListInUse"))%>'.replace(/%1/, '<%=mbrGrpsNotDeleted%>'));
