...
<%
   ...
   Vector orgs = urdbtemp2.getOrgEntityUserAdmin();
   Vector mgab = new Vector();
   Vector name = new Vector();
   
   for (int i=0; i < orgs.size(); i++) {
   
   	String temp = (String) orgs.elementAt(i);
   	OrgEntityDataBean oedb = new OrgEntityDataBean();
   	oedb.setDataBeanKeyMemberId(temp);
   	oedb.populate();
   	...
   	Enumeration enum1 = mgabtemp.findByMember(new Long(temp));
   	
   	while (enum1.hasMoreElements()) {
   		mgab.addElement((MemberGroupAccessBean) enum1.nextElement());
   		name.addElement(oedb.getOrganizationName());
   	}
...
{  
    ...
    parent.put("<%=SegmentConstants.PARAMETER_USER_IDS%>","<%=memberId%>");
