          ...
          criteria = true;
		    ...
		    if ("<%=orgId%>" != null || "<%=orgId%>" != "") {
			    	//for (var j=0; j < document.f1.SelectOrg.length; j++) {
				 	// 	if ("<%=orgId%>" == document.f1.SelectOrg[j].value) {
...
function LUS_SelectResultItem()
{
   ...
   var webAppPath = "<%= UIUtil.getWebappPath(request) %>";
   var newURL = top.getWebappPath() + "BuyMemberGroupCriteriaView?segmentId=<%= mbrGrpId %>&memberGroupType=<%= memberGroupType %>&oid=" + selectedItem;
   top.showProgressIndicator(true);       
...
function RolesHelper_Setup()
{
   ...
   with (document.f1) {
 	  	...
 	  	iframeElement.id = 'OABuyUserRoles_RolesHelper_dataFrame';
 	        iframeElement.width = '0';
 	        iframeElement.height = '0';
 	        iframeElement.src = webAppPath + "OAGetRolesForOrgView?oid=<%= oid %>";
 	        iframeElement.title = '';
 	        iframeElement.style.display = 'none';
 	    	document.body.appendChild(iframeElement);
...
%>    
  ...
  %>			
		...
		<INPUT TYPE="HIDDEN" NAME="memberGroupType" VALUE="<%= memberGroupType %>">	
